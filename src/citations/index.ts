...
export class ModCitations {
    editor: Editor
    citationType: string
    fnOverrideElement: HTMLElement | null
    citRenderer: any
    ...
    constructor(editor: Editor) {
        ...
        this.editor = editor
        this.citationType = ""
        this.fnOverrideElement = false as unknown as null
    ...
    recoverAfterCiteProcCrash(): Promise<void> {
        console.warn(
        ...
        citationNodes.forEach(citation => {
            try {
                ...
                const referencesData = JSON.parse(
                    (citation as HTMLElement).dataset.references || "[]"
                )
            ...
            try {
                ...
                allCitations.forEach(citation => {
                    try {
                        ...
                        refs.forEach((ref: {id: number}) => allRefs.add(ref.id))
                    } catch (_error) {
                ...
                const bibDB = (this.editor.mod.db as any).bibDB.db as any
                Array.from(allRefs)
                    .sort()
                    .forEach(id => {
                        const item = bibDB[id]
                ...
                `
                ;(styleEl as HTMLElement).innerHTML = basicCSS
    ...
    checkTrackingDialog(): Promise<void> {
        if (!this.editor.view.state.doc.attrs.tracked) {
            return Promise.resolve()
        }
        const buttons: DialogButtonSpec[] = []
        let dialog: InstanceType<typeof Dialog>
        const promise = new Promise<void>(resolve => {
            buttons.push({
                type: "cancel",
                click: () => {
                    dialog.close()
                    resolve(cancelPromise() as unknown as void)
                }
            })
            buttons.push({
                type: "ok",
    ...
    layoutCitationsTwo(): void {
        ...
        if ((styleEl as HTMLElement).innerHTML !== css) {
            ;(styleEl as HTMLElement).innerHTML = css
        }
        ...
        if (this.citationType === "note") {
            ...
            if (emptyCitations) {
                ...
                footnoteCitTexts.forEach((citText: string, index: number) => {
        ...
        if (needFootnoteLayout) {
            ;((this.editor.mod as any).footnotes.layout as any).updateDOM()
        }
    }

    footnoteNumberOverride(): void {
