...
function getReplaceStep(fromDoc: Node, toDoc: Node): ReplaceStep | false {
    ...
    return new ReplaceStep(start, endB, toDoc.slice(start, endA) as Slice)
}
...
class RecreateTransform {
    ...
    addSetNodeMarkup(): void {
        ...
        if (start !== null) {
            const fromNode = fromDoc.nodeAt(start) as Node,
                toNode = toDoc.nodeAt(start) as Node
            try {
    ...
    simplifyTr(): void {
        ...
        while (oldSteps.length) {
            let step: any = oldSteps.shift()
            while (oldSteps.length && step.merge(oldSteps[0])) {
                const addedStep: any = oldSteps.shift()
                if (
