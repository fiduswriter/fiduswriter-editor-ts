...
export class MergeEditor {
    ...
    mergeDialog: Dialog
    warningDialog: Dialog | false
    db: {bibliography: BibDB; images: ImageDB}
    ...
    diffEditorPlugins: Array<[any, ((doc: Node) => any)?]>
    offStepsNotTracked: Step[]
    ...
    constructor(
        ...
        this.mergeView1 = null as unknown as EditorView
        this.mergeView2 = null as unknown as EditorView
        this.mergeView3 = null as unknown as EditorView
        ...
        const schemaCached = (this.schema as any).cached
        ...
        schemaCached.decryptImage = (this.editor.schema as any).cached
            ?.decryptImage
    }
    ...
    init(): void {
        ...
        const offlineChangeset = new changeSet(
            this.offlineTr as unknown as Transaction
        ).getChangeSet()
        const onlineChangeset = new changeSet(
            this.onlineTr as unknown as Transaction
        ).getChangeSet()
    ...
    createMergeDialog(offlineTr: Transaction, onlineDoc: Node): Dialog {
        const buttons = [
            {
                            ...
                            buttons: [
                                {
                                    ...
                                    click: () => {
                                        this.startMerge(offlineTr, onlineDoc)
                                        ;(this.warningDialog as Dialog).close()
                                    }
                                }
                            ]
                        })
    ...
    markChangesinDiffEditor(
        changeset: {changes: readonly any[]},
        view: EditorView,
    ...
    bindEditorView(elementId: string, doc: Node): EditorView {
    ...
    renderCitation(view: EditorView, elementId: string): void {
    ...
    startMerge(offlineTr: Transaction, onlineDoc: Node): void {
        try {
            ...
            this.offStepsNotTracked.forEach(markstep => {
                ...
                if (
                    ...
                    ;(
                        offlineRebaseMapping as unknown as {
            ...
            ;(this.editor.mod.collab as any).doc.merge.handleMergeFailure(
                error as Error,
                offlineDoc,
                onlineDoc,
                this
    ...
    unHideSections(view: EditorView): void {
    ...
    applyChangesToMainEditor(onlineDoc: Node, mergedDoc: Node): void {
        ...
        const tr = simplifyTransform(
            recreateTransform(onlineDoc, mergedDoc) as unknown as Transaction
        )
        const OnlineStepsLost = recreateTransform(
            onlineDoc,
            this.editor.view.state.doc
        )
        const onlineStepsLostChangeset = new changeSet(
            OnlineStepsLost as unknown as Transaction
        )
        ...
        if (conflicts.length > 0) {
            ...
            const maps = new Mapping(
                ([] as import("prosemirror-transform").StepMap[])
                    .concat(
                        tr.mapping.maps
                            .slice()
                            .reverse()
            ...
            ;(this.editor.mod.collab as any).doc.footnoteRender = true
    ...
    findNotTrackedSteps(tr: Transaction, trackedSteps: number[]): Step[] {
        /* Find steps not tracked by PM, usually steps that cause change of attrs */
        const nonTrackedSteps: Step[] = []
        tr.steps.forEach((step, index) => {
    ...
    updateDB(
        ...
        usedBibs.forEach(id => {
            if (!this.editor.mod.db!.bibDB.db[id] && oldBibDB[id]) {
                ;(this.editor.mod.db!.bibDB as any).updateReference(
                    id,
                    oldBibDB[id]
                )
            }
        ...
        ;(usedImages as unknown as number[]).forEach(id => {
            if (!this.editor.mod.db!.imageDB.db[id] && oldImageDB[id]) {
                ...
                if (
                    ...
                    ;(this.editor.mod.db!.imageDB as any).setImage(
                        id as number,
                        oldImageDB[id]
                    )
                } else {
                    ...
                    ;(this.editor.mod.db!.imageDB as any)
                        .reUploadImage(
                            id as number,
                        ...
                        .then(
                            ({id, newId}: {id: number; newId: number}) => {
