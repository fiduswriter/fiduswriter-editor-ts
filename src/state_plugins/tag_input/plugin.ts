...
interface TagInputState {
    tagsPartPositions: Array<{start: number; end: number}>
}

export const tagInputPlugin = (options: {editor: Editor}) =>
    new Plugin<TagInputState>({
        key,
        state: {
            init(_config, state) {
                ...
                if (options.editor.docInfo.access_rights === "write") {
                    self.spec.props.nodeViews["tags_part"] = (
                        node: Node,
                        view: EditorView,
                        getPos: () => number
        ...
        appendTransaction: (trs, oldState, newState) => {
            ...
            const pluginState = key.getState(newState) as TagInputState
