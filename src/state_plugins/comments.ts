...
export const addCommentDuringCreationDecoration = (
    state: EditorState,
    tr: Transaction
): Transaction | false => {
...
export const removeCommentDuringCreationDecoration = (
    state: EditorState,
    tr: Transaction
): Transaction | false => {
    let {decos} = key.getState(state) as {decos: DecorationSet}
...
export const commentsPlugin = (options: CommentPluginOptions) =>
    new Plugin({
        key,
        state: {
            init() {
        ...
        appendTransaction: (trs, _oldState, state) => {
            ...
            const deletedComments: Record<string, number> = {}
            ...
            let steps: (AddMarkStep | undefined)[] = []
            let tr = state.tr
            ...
            steps.forEach(step => tr.step(step as AddMarkStep))
            return tr
        },
        props: {
