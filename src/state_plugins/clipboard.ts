...
export const getPasteRange = (state: EditorState): number[] | null => {
    const pluginState = key.getState(state) as {pasteRange: number[] | null}
...
const filterFragment = (
    fragment: Fragment,
    allowedElements: string[] | false,
    allowedMarks: string[] | false,
        ...
        state: {
            ...
            apply(tr, _prev, oldState) {
                ...
                if (uiEventMeta && ["paste", "drop"].includes(uiEventMeta)) {
                    ...
                    pasteRange = (
                        key.getState(oldState) as {pasteRange: number[] | null}
                    )?.pasteRange
