...
import type {DocInfo, EditorUser} from "../types.js"

export interface MarginBoxFilterOptions {
    track?: boolean
    comments?: boolean
    info?: boolean
    help?: boolean
...
export interface CommentMarginBox {
    type: "comment"
    data: Comment
    view: string
    active: boolean
...
export interface TrackMarginBox {
    type: "insertion" | "deletion" | "format_change" | "block_change"
    data: TrackData
    node: Node
    view: string
    active: boolean
...
export interface HelpMarginBox {
    type: "help"
    data: {active?: boolean; help: unknown}
    active?: boolean
}

export interface WarningMarginBox {
    type: "warning"
    data: {active?: boolean; warning: string}
    active?: boolean
}
...
const helpTemplate = ({
    ...
    help: {active?: boolean; help: unknown}
...
const warningTemplate = ({
    ...
    warning: {active?: boolean; warning: string}
    filterOptions: MarginBoxFilterOptions
}) => {
...
const blockChangeTemplate = (
    {before}: {before: {type: string; attrs?: {order?: number}}},
    node: Node
) =>
...
export const globalCommentsTemplate = ({
    ...
    globalComments: CommentMarginBox[]
    editComment: boolean
    activeCommentAnswerId: number | string | undefined
    user: EditorUser
    docInfo: DocInfo
...
function getAssignees(docInfo: DocInfo): ContactPerson[] {
    return docInfo.owner ? [docInfo.owner, ...docInfo.owner.contacts] : []
}

export const marginBoxOptions = (
    comment: MarginBoxOptionComment,
    user: EditorUser,
    docInfo: DocInfo
) => {
                ...
                <li>
                    ...
                    <div class="fw-pulldown marginbox-options-submenu">
                        ...
                        ${getAssignees(docInfo)
                            ...
                            .map(
                                contact =>
                                    `<li><span class="fw-pulldown-item assign-comment" data-id="${comment.id}" data-user="${contact.id}" data-username="${escapeText(contact.name || "")}" title="${gettext("Assign comment to")} ${escapeText(contact.name || "")}">${escapeText(contact.name || "")}</span></li>`
                            )
                            .join("")}
                        </ul>
                    </div>
