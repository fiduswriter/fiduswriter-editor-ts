...
import type {Editor, EditorUser} from "../types.js"
...
function markInsertion(
    ...
    tr.doc.nodesBetween(from, to, (node, pos) => {
        ...
        const track = (node.attrs.track as Track[] | undefined)?.slice() || []
        if (!approved) {
            track.push({
                type: "insertion",
                user: user.id,
...
function markDeletion(
    ...
    tr.doc.nodesBetween(from, to, (node, pos) => {
        if (pos < from && node.type.name === "table_cell") {
            ...
            !(node.attrs.track as Track[] | undefined)?.find(
            ...
            if (insertionTrack) {
                ...
                const track = (node.attrs.track as Track[]).slice()
                track.push({
                    type: "deletion",
                    user: user.id,
                    username: user.username,
...
function markWrapping(
    ...
    const oldTrack = oldNode.attrs.track as Track[] | undefined
    if (!oldTrack || !newNode) {
        return
    }
    ...
    if (blockTrack) {
        ...
        const before = blockTrack.before as {
            type: string
            attrs: Record<string, unknown>
        }
        if (
            before.type !== newNode.type.name ||
        ...
        const before = {
            ...
            attrs: Object.assign({}, oldNode.attrs) as Record<string, unknown>
        }
        if (before.attrs.id) {
...
export function amendTransaction(
    ...
    const trMeta = (tr as unknown as {meta?: Record<string, unknown>}).meta
    if (
        !tr.steps.length ||
        (trMeta &&
...
export function trackedTransaction(
    tr: Transaction,
    state: EditorState,
    user: EditorUser,
    approved: boolean,
        ...
        if (step instanceof ReplaceStep) {
            ...
            doc.nodesBetween(step.from, step.to, (node, pos) => {
                ...
                if (
                    ...
                    if (formatChangeMark) {
                        if (
                            (formatChangeMark.attrs.after as string[]).includes(
                                step.mark.type.name
                            )
                        ) {
