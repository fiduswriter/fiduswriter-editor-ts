...
export class Editor {
    ...
    constructor(
        ...
        ;(this.schema as any).cached.decryptImage = (
            imageEntry: {image: string; original_file_type?: string},
            _dom: HTMLElement
        ) => {
    ...
    init(): Promise<void> {
        ...
        return Promise.all(initPromises)
            ...
            .then(() => {
                activateWait(true)
                const stylesPayload: Record<string, unknown> = {
                    id: this.docInfo.id
                }
                        ...
                        receiveData: (data: Record<string, any>) => {
                            ...
                            switch (data.type) {
                                ...
                                case "diff":
                                    ...
                                    this.mod.collab.doc.receiveDiff(data as any)
                                    break
                                case "confirm_diff":
    ...
    startWebSocket(): void {
        ...
        this.ws.ws.addEventListener("close", () => {
            if (this.menu.toolbarViews) {
                this.menu.toolbarViews.forEach((view: any) => view.update())
            }
            if (this.menu.headerView) {
                ;(this.menu.headerView as any).update()
            }
        })
    }
    ...
    async _createE2EEDocument(): Promise<void> {
        ...
        if (hasPassphraseKeys) {
            ...
            if (!PassphraseManager.hasKeysInSession()) {
                ...
                })) as {action: string; passphrase?: string}
        ...
        if (!password!) {
            ...
            password = await new Promise(resolve => {
                createPasswordDialog((pwd: string) => resolve(pwd))
            })
        }
    ...
    close(): void {
        if (this.menu.toolbarViews) {
            this.menu.toolbarViews.forEach((view: any) => view.destroy())
        }
        if (this.menu.selectionMenuViews) {
            this.menu.selectionMenuViews.forEach((view: any) => view.destroy())
        }
        if (this.menu.headerView) {
        ...
        if (this.e2ee && this.view) {
            ...
            if (titleNode) {
                let title = ""
                ;(titleNode as any).forEach((child: any) => {
                    if (
                        !child.marks.find(
                            (mark: any) => mark.type.name === "deletion"
                        )
                    ) {
    ...
    onResize(): void {
        ...
        this.menu.toolbarViews?.forEach((view: any) => view.onResize())
    }
    ...
    initEditor(): void {
        ...
        this.view = new EditorView(
            this.dom.querySelector("#document-editable") as HTMLElement,
            {
                ...
                return Promise.all(
                    Object.values(plugin).map(pluginExport => {
                        if (typeof pluginExport === "function") {
                            ...
                            this.plugins![name] = new (pluginExport as any)(
                                this
                            )
                            return (
    ...
    // filters
    getDoc(options: {use_current_view?: boolean; changes?: string} = {}): any {
        const doc =
            this.app.isOffline() || Boolean(options.use_current_view)
                ? (this.view as any).docView.node
        ...
        let title = ""
        ;(pmDoc.firstChild as any).forEach((child: any) => {
            if (
                !child.marks.find((mark: any) => mark.type.name === "deletion")
            ) {
                title += child.textContent
            }
        })
    ...
    scrollIdIntoView(id: string): void {
        let foundPos: number | false = false
        let view: EditorView
        ...
        this.view.state.doc.descendants((node, pos) => {
            if (foundPos !== false) {
                ...
                ((node.type as any).groups.includes("heading") ||
        ...
        if (foundPos === false) {
            const fnView = this.mod.footnotes.fnEditor.view as EditorView
    ...
    scrollPosIntoView(pos: number, view: EditorView): void {
        const topMenuHeight =
            (this.dom.querySelector("header") as HTMLElement).offsetHeight + 10
    ...
    scrollBibliographyIntoView(): void {
        const topMenuHeight =
            (this.dom.querySelector("header") as HTMLElement).offsetHeight + 10
