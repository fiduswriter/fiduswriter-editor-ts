...
export class ModFootnoteEditor {
    mod: FootnoteMod
    schema: Schema
    fnStatePlugins: [any, ((doc: Node) => any)?][]
    view!: EditorView

    constructor(mod: FootnoteMod) {
        mod.fnEditor = this
        this.mod = mod
        this.schema = fnSchema as Schema
    ...
    init(): void {
        ...
        this.view = new EditorView(
            document.getElementById("footnote-box-container") as HTMLElement,
            {
                ...
                dispatchTransaction: tr => {
                    ...
                    if (
                        tr.docChanged &&
                        (tr.steps[0] as any).jsonID === "replace" &&
                        (tr.steps[0] as any).from === 0 &&
                        (tr.steps[0] as any).to === 0 &&
                        !remote &&
                        !fromMain
                    ) {
                        return
                ...
                ...new Set(
                    tr.steps
                        .map((step, index) => {
                            ...
                            // the main editor.
                            const $pos = tr.docs[index].resolve(
                                (step as any).from
                            )
                            if ($pos.depth === 0) {
                                return -1
    ...
    applyDiffs(diffs: any[], cid: number): void {
    ...
    removeFootnote(index: number, tr: Transaction): void {
        if ((this.mod.editor.mod.collab as any).doc.receiving) {
            return
        }
