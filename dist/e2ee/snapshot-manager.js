export class E2EESnapshotManager {
    ...
    async getEncryptedSnapshot() {
        ...
        const encryptedContent = await E2EEEncryptor.encryptObject(content, this.key);
        const encryptedComments = await E2EEEncryptor.encryptObject(comments, this.key);
        const encryptedBibliography = await E2EEEncryptor.encryptObject(bibliography, this.key);
    ...
    async handleSnapshotReceived(message) {
        ...
        if (message.e2ee_salt) {
            ...
            showSystemMessage(gettext("The document password was changed by another user. You will need to enter the new password to continue editing."));
        }
    }
    /**
    ...
    async reEncryptWithNewKey(newKey, newSaltBase64, newIterations) {
        ...
        const encryptedContent = await E2EEEncryptor.encryptObject(content, newKey);
        const encryptedComments = await E2EEEncryptor.encryptObject(comments, newKey);
        const encryptedBibliography = await E2EEEncryptor.encryptObject(bibliography, newKey);
