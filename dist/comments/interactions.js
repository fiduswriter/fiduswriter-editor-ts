...
import { deactivateAllSelectedChanges, getCommentDuringCreationDecoration } from "../state_plugins/index.js";
...
export class ModCommentInteractions {
        ...
        this.mod.editor.dom.addEventListener("click", event => {
            ...
            switch (true) {
                ...
                case findTarget(event, ".edit-comment-answer", el):
                    this.editComment = false;
                    this.editAnswer(el.target.dataset.id, el.target.dataset.answer);
                    break;
                ...
                case findTarget(event, ".assign-comment", el):
                    this.assignComment(el.target.dataset.id, Number.parseInt(el.target.dataset.user), el.target.dataset.username);
                    break;
                ...
                case findTarget(event, ".delete-comment-answer", el):
                    this.deleteCommentAnswer(el.target.dataset.id, el.target.dataset.answer);
                    break;
                default:
                    break;
            }
    ...
    initEditor() {
        const commentEditorDOM = document.querySelector("#comment-editor"), answerEditorDOM = document.querySelector("#answer-editor");
        if (commentEditorDOM?.matches(":not(:empty)") ||
        ...
        if (commentEditorDOM) {
            ...
            this.editor = new CommentEditor(this.mod, id, commentEditorDOM, value.text, { isMajor: value.isMajor, isGlobal });
        }
        else {
            const answerId = this.activeCommentAnswerId, text = answerId
                ? this.mod.store.comments[id].answers.find((answer) => answer.id === answerId).answer
                : [];
            this.editor = new CommentAnswerEditor(this.mod, id, answerEditorDOM, text, { answerId: answerId ? String(answerId) : undefined });
        }
        this.editor.init();
    }
    updateDOM() {
            ...
            this.editor.dom.childNodes.forEach((node) => {
                ...
                if (el.classList?.contains("comment-btns")) {
                    el.childNodes.forEach(buttons => {
                        const buttonEl = buttons;
                        if (buttonEl.classList?.contains("submit") &&
        ...
        if (id === "-1") {
            ...
            if (this.creatingGlobalComment) {
                ...
                this.mod.store.addComment({
                    ...
                    comment,
                    isMajor
                }, referrer.from, referrer.to, this.mod.store.commentDuringCreation.view);
            }
        }
        else {
