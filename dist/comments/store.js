...
import { addCommentDuringCreationDecoration, removeCommentDuringCreationDecoration } from "../state_plugins/index.js";
...
export class ModCommentStore {
    ...
    mustSend() {
        ...
        window.setTimeout(() => this.mod.editor.mod.collab.doc.sendToCollaborators(), 100);
    }
    ...
    addCommentDuringCreation(view) {
        const state = view.state, tr = addCommentDuringCreationDecoration(state, state.tr);
        if (!tr) {
    ...
    addComment(commentData, posFrom, posTo, view) {
        const id = randomCommentId(), markType = view.state.schema.marks.comment.create({ id }), tr = this.addMark(view.state.tr, posFrom, posTo, markType);
        if (tr) {
            commentData.id = id;
            commentData.answers = [];
    ...
    addLocalComment(commentData, local) {
        ...
        if (commentData.user !== undefined &&
            !this.mod.editor.mod.collab.pastParticipants.find((participant) => participant.id === commentData.user)) {
            ;
        ...
        if (commentData.assignedUser &&
            !this.mod.editor.mod.collab.pastParticipants.find((participant) => participant.id === commentData.assignedUser)) {
            ;
            this.mod.editor.mod.collab.pastParticipants.push({
    ...
    removeCommentMarks(id) {
        ...
        [
            ...
            const tr = view.state.tr, markType = view.state.schema.marks.comment.create({ id });
            view.state.doc.descendants((node, pos) => {
                const nodeStart = pos, nodeEnd = pos + node.nodeSize;
    ...
    addLocalAnswer(id, answer, local) {
        if (this.comments[id]) {
            ...
            const answerFound = this.comments[id].answers.find((existingAnswer) => existingAnswer.id === answer.id);
            if (!answerFound) {
    ...
    updateLocalAnswer(id, answerId, answerText, local) {
        if (this.comments[id]?.answers) {
            const answer = this.comments[id].answers.find((existingAnswer) => existingAnswer.id === answerId);
            if (answer) {
    ...
    unsentEvents() {
        ...
        for (let i = 0; i < this.unsent.length; i++) {
            ...
            if (event.type == "delete") {
                ...
                if (foundAnswer) {
                    result.push(Object.assign({}, foundAnswer, {
                        type: "add_answer",
                        id: event.id,
