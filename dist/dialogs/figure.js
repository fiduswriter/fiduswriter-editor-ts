...
export class FigureDialog {
    ...
    layoutMathEditor() {
        ...
        this.mathliveDOM = dialogEl.querySelector(".math-field");
        this.nonMathElements = [
            dialogEl.querySelector("#insert-figure-image"),
            dialogEl.querySelector(".formula-or-figure")
        ];
    ...
    showPlaceHolder() {
        if (this.mathField &&
            ...
            this.mathliveDOM.insertAdjacentHTML("beforeend", `<span class="fw-placeholder" >${gettext("Type formula")}</span>`);
        }
    }
    hidePlaceHolder() {
    ...
    selectImage() {
        const imageSelection = new ImageSelectionDialog(this.imageDB, this.userImageDB, this.imgId, this.editor);
        imageSelection
            .init()
            .then((result) => {
            const { id, db } = result;
            if (id) {
                ...
                this.copyright =
                    db === "document"
                        ? this.imageDB.db[String(id)].copyright || false
                        : JSON.parse(JSON.stringify(this.userImageDB.db[String(id)]
                            .copyright));
    ...
    async layoutImagePreview() {
        if (this.imgId) {
            ...
            if (imageEntry?.file_type === "application/octet-stream") {
                ...
                if (key) {
                    try {
                        imgSrc = await E2EEEncryptor.decryptImageToUrl(String(imageEntry.image), key, String(imageEntry.original_file_type || "image/png"));
                    }
                    catch (_e) {
            ...
            const dialogEl = this.dialog.dialogEl;
            dialogEl.querySelector(".inner-figure-preview").innerHTML = `<img src="${imgSrc}" style="max-width: 400px;max-height:220px">
    ...
    async submitForm() {
        ...
        if (this.imgDb === "user") {
            if (this.editor.e2ee?.encrypted) {
                ...
                try {
                    ...
                    const file = new File([blob], String(userImage.image).split("/").pop() || "image", { type: blob.type });
                    const key = this.editor.e2ee.key;
                    if (!key) {
                        throw new Error("E2EE key missing");
                    }
                    const encryptedFile = await E2EEEncryptor.encryptImage(file, key);
                    const encryptedCopyright = await E2EEEncryptor.encryptObject(this.copyright, key);
                ...
                const imageEntry = JSON.parse(JSON.stringify(this.userImageDB.db[String(this.imgId)]));
            ...
            !deepEqual(this.copyright, this.imageDB.db[String(this.imgId)].copyright)) {
            const imageEntry = JSON.parse(JSON.stringify(this.imageDB.db[String(this.imgId)]));
            imageEntry.copyright = this.copyright;
            this.imageDB.setImage(this.imgId, imageEntry);
        }
        const node = this.node;
        const currentEquationNode = node
            ? node.content.content.find((node) => node.type.name === "figure_equation")
            : undefined;
        const currentImageNode = node
            ? node.content.content.find((node) => node.type.name === "image")
            : undefined;
        if (node &&
            this.insideFigure &&
            this.equation ===
        ...
        const content = [];
        if (this.imgId) {
            content.push(this.editor.currentView.state.schema.nodes["image"].create({
                image: this.imgId
            }));
        }
        else {
            content.push(this.editor.currentView.state.schema.nodes["figure_equation"].create({
                equation: this.equation
            }));
        }
        const captionNode = node?.content?.content.find((node) => node.type.name === "figure_caption") ||
        ...
        const tr = this.editor.currentView.state.tr.replaceSelectionWith(this.editor.currentView.state.schema.nodes["figure"].createAndFill({
    ...
    init() {
        ...
        if (node?.type && node.type.name === "figure") {
            ...
            this.equation =
                node.content.content.find((childNode) => childNode.type.name === "figure_equation")?.attrs.equation || "";
            this.imgId =
                node.content.content.find((childNode) => childNode.type.name === "image")?.attrs.image || false;
            this.imgDb = "document";
            this.category = node.attrs.category;
            this.caption = node.attrs.caption;
            this.aligned = node.attrs.aligned;
        ...
        this.dialog.open();
        const alignmentSelector = dropdownSelect(this.dialog.dialogEl.querySelector(".figure-alignment"), {
            onChange: newValue => {
        ...
        const dialogEl = this.dialog.dialogEl;
        const figureWidthDOM = dialogEl.querySelector(".figure-width");
        figureWidthDOM.style.width = "80%";
        const widthLabel = figureWidthDOM.firstElementChild;
        ...
        dropdownSelect(dialogEl.querySelector(".figure-category"), {
            onChange: newValue => {
        ...
        dropdownSelect(dialogEl.querySelector(".figure-caption"), {
            onChange: newValue => {
        ...
        if (this.imgId && this.imageDB.db[String(this.imgId)]) {
            this.copyright = this.imageDB.db[String(this.imgId)].copyright || false;
            this.layoutImagePreview();
        }
        else {
            this.layoutMathEditor();
