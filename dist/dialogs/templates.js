...
export const linkDialogTemplate = ({ defaultLink, internalTargets, linkType, title, target, allowedContent }) => `${allowedContent.cross_reference && internalTargets.length
    ? `<div class="fw-radio">
        ...
        <div class="fw-select-container">
            <select class="cross-reference-selector fw-button fw-light fw-large" required="">
                ...
                ${internalTargets
        .map(iTarget => `<option class="cross-reference-item" type="text" value="${iTarget.id}" ${target === iTarget.id ? "selected" : ""}>
    ...
    ? `<div class="fw-radio">
        ...
        <div class="fw-select-container">
            <select class="internal-link-selector fw-button fw-light fw-large" required="">
                ...
                ${internalTargets
        .map(iTarget => `<option class="link-item" type="text" value="${iTarget.id}" ${target === iTarget.id ? "selected" : ""}>
                            ${escapeText(iTarget.text)}
                        </option>`)
        .join("")}
            </select>
...
export const pdfExportDialogTemplate = () => `
    ...
    <p>
        ...
        ${infoTooltip(gettext("Moves figures to the top of the page instead of interrupting the text flow. A custom document style can override this."), "pdf-figure-page-floats-help")}<br>
    ...
    <p>
        ...
        ${infoTooltip(gettext("Creates a self-contained, standards-based PDF suited for long-term archiving. If files are embedded in the PDF, PDF/A-4f is used, as PDF/A-4 does not allow embedded files."), "pdf-pdf-a-help")}<br>
        <label><input type="checkbox" class="pdf-pdf-ua" aria-describedby="pdf-pdf-ua-help"> ${gettext("PDF/UA (accessible, tagged)")}</label>
        ${infoTooltip(gettext("Tags the PDF with a logical structure tree so screen readers can navigate the document."), "pdf-pdf-ua-help")}
    </p>
    <p>
        <label>
        ...
        ${infoTooltip(gettext("Attaches the original HTML of the document to the PDF. Like an embedded Fidus Writer file, this requires the PDF/A-4f variant."), "pdf-embed-source-help")}
    </p>
    ...
    <p>
        ...
        ${infoTooltip(gettext("Small lines at the corners of the page that show printers where to cut the paper."), "pdf-crop-marks-help")}<br>
        <label><input type="checkbox" class="pdf-trim-box" aria-describedby="pdf-trim-box-help"> ${gettext("Trim box")}</label>
        ${infoTooltip(gettext("Marks the final size of the page after it has been cut."), "pdf-trim-box-help")}<br>
        <label><input type="checkbox" class="pdf-bleed-box" aria-describedby="pdf-bleed-box-help"> ${gettext("Bleed box")}</label>
        ${infoTooltip(gettext("Marks the area beyond the final page size where images and colors must extend so no white edges appear after cutting."), "pdf-bleed-box-help")}<br>
        ...
        ${infoTooltip(gettext("How far, in millimeters, images and colors extend beyond the edge of the page."), "pdf-bleed-mm-help")}<br>
        ...
        ${infoTooltip(gettext("Draws a visible border around hyperlinks in the PDF so they are easier to find."), "pdf-link-borders-help")}<br>
 ...
 */
export const exportTrackChangesTemplate = (name) => `
    <h4>${gettext("Tracked changes")}</h4>
    <div class="fw-radio">
...
export const getExportTrackChangesValue = (dialogEl, name) => {
    return dialogEl.querySelector(`input[name="${name}"]:checked`)?.value || "resolve";
};
export const htmlExportDialogTemplate = () => `
...
`;
export const epubExportDialogTemplate = () => `
...
export const tableConfigurationTemplate = ({ language }) => `<table class="fw-dialog-table">
        <tbody>
            <tr>
...
export const orderedListStartTemplate = ({ order }) => `<div title="${gettext("List start")}">
        <p><input class="list-start" type="number" name="list-start" min="1" value="${order}"></p>
    </div>`;
export const mathDialogTemplate = () => `<div class="math-field-outer">
        <div class="math-field" type="text" name="math" ></div>
    </div>`;
export const figureImageItemTemplate = ({ id, cats, image, thumbnail, title }) => `<tr id="Image_${id}" class="${cats.map(cat => `cat_${escapeText(cat)} `).join("")}" >
...
export const figureImageTemplate = ({ imageDB }) => `<div>
...
export const configureFigureTemplate = ({ language }) => `<div class="fw-media-uploader">
                    ...
                    <tr>
                        ...
                        <td>
                            <select class="figure-category">
                                <option value="none">${gettext("None")}</option>
                                ${Object.entries(CATS)
    .map(([id, titleObject]) => `<option value="${id}">${titleObject[language]}</option>`)
    .join("")}
                            </select>
                        </td>
                    </tr>
...
export const configureCitationTemplate = ({ citedItemsHTML, citeFormat }) => `<div id="my-sources" class="fw-ar-container">
...
export const selectedCitationTemplate = ({ title, author, year, id, db, prefix, locator }) => `<tr id="selected-source-${db}-${id}" class="selected-source">
...
export const languageTemplate = ({ currentLanguage, allowedLanguages }) => `<select class="fw-button fw-light fw-large">
        ${allowedLanguages
    .map(language => `<option value="${language[0]}" ${language[0] === currentLanguage ? "selected" : ""}>
                    ${language[1]}
                </option>`)
    .join("")}
    </select>`;
