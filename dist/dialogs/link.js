...
export class LinkDialog {
    ...
    init() {
        ...
        this.internalTargets = getInternalTargets(this.editor.view.state, this.editor.view.state.doc.attrs.language, "main").concat(getInternalTargets(this.editor.mod.footnotes.fnEditor.view.state, this.editor.view.state.doc.attrs.language, "foot"));
        this.createDialog();
    }
    // Check if there is an existing link at the selection. If this is the case
    // use its values in dialog.
    checkLink() {
        const state = this.editor.currentView.state, from = state.selection.from, linkMark = state.selection.$from
            .marks()
    ...
    checkCrossReference() {
        const node = this.editor.currentView.state.selection
            .node;
    ...
    extendSelectionToMark(pos, mark) {
        const view = this.editor.currentView, state = view.state, $pos = state.doc.resolve(pos);
        let startIndex = $pos.index(), endIndex = $pos.indexAfter();
        while (startIndex > 0 &&
        ...
        let startPos = $pos.start(), endPos = startPos;
        ...
        view.dispatch(state.tr.setSelection(TextSelection.create(state.doc, startPos, endPos)));
    }
    createDialog() {
        const buttons = [];
        buttons.push({
            ...
            classes: "fw-dark",
            click: () => {
                const linkTypeEl = this.dialog.dialogEl.querySelector("input[name=link-type]:checked"), linkType = linkTypeEl ? linkTypeEl.value : "external";
                let href, title, target = false, change = false;
                switch (linkType) {
                    case "internal": {
                ...
                const view = this.editor.currentView, posFrom = view.state.selection.from, tr = view.state.tr;
        ...
        const initialFocus = this.linkType === "cross_reference"
        ...
        if (this.internalTargets.length) {
            const externalEls = this.dialog.dialogEl.querySelectorAll("input.link, input.link-title"), internalEls = this.dialog.dialogEl.querySelectorAll("select.internal-link-selector"), crossReferenceEls = this.dialog.dialogEl.querySelectorAll("select.cross-reference-selector"), externalSwitchers = this.dialog.dialogEl.querySelectorAll("input.link, input.link-title, label.link-external-label, input.link-external-check"), internalSwitchers = this.dialog.dialogEl.querySelectorAll("select.internal-link-selector, label.link-internal-label, input.link-internal-check"), crossReferenceSwitchers = this.dialog.dialogEl.querySelectorAll("select.cross-reference-selector, label.cross-reference-label, input.cross-reference-check"), radioInternal = this.dialog.dialogEl.querySelector("input.link-internal-check"), radioExternal = this.dialog.dialogEl.querySelector("input.link-external-check"), radioCrossReference = this.dialog.dialogEl.querySelector("input.cross-reference-check");
            switch (this.linkType) {
                case "internal":
                    externalEls.forEach(el => el.classList.add("fw-disabled"));
                    crossReferenceEls.forEach(el => el.classList.add("fw-disabled"));
            ...
            internalSwitchers.forEach(el => el.addEventListener("click", () => {
                externalEls.forEach(el => el.classList.add("fw-disabled"));
                internalEls.forEach(el => el.classList.remove("fw-disabled"));
                crossReferenceEls.forEach(el => el.classList.add("fw-disabled"));
                radioInternal.checked = true;
            }));
            externalSwitchers.forEach(el => el.addEventListener("click", () => {
                internalEls.forEach(el => el.classList.add("fw-disabled"));
                externalEls.forEach(el => el.classList.remove("fw-disabled"));
                crossReferenceEls.forEach(el => el.classList.add("fw-disabled"));
                radioExternal.checked = true;
            }));
            crossReferenceSwitchers.forEach(el => el.addEventListener("click", () => {
                internalEls.forEach(el => el.classList.add("fw-disabled"));
                externalEls.forEach(el => el.classList.add("fw-disabled"));
                crossReferenceEls.forEach(el => el.classList.remove("fw-disabled"));
                radioCrossReference.checked = true;
            }));
        }
    }
