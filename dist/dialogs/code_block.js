...
export class CodeBlockDialog {
    ...
    getEnabledCategories() {
        const codeCategories = this.editor.view.state.doc.attrs.code_categories || {};
        const categories = [];
    ...
    init() {
        ...
        if (this.node &&
            this.node.attrs.track?.find((track) => track.type === "deletion")) {
        ...
        this.dialog = new Dialog({
            ...
            body: this.getDialogTemplate(language, enabledCategories, availableLanguages),
            buttons,
            onClose: () => this.editor.currentView.focus()
        });
        this.dialog.open();
        // Language selector
        const languageSelector = dropdownSelect(this.dialog.dialogEl.querySelector(".code-block-language"), {
            onChange: (newValue) => {
                if (newValue !== false) {
        ...
        if (enabledCategories.length > 0) {
            const categorySelector = dropdownSelect(this.dialog.dialogEl.querySelector(".code-block-category"), {
                onChange: (newValue) => {
                    if (newValue !== false) {
        ...
        // Title input
        const titleInput = this.dialog.dialogEl.querySelector(".code-block-title");
        if (titleInput) {
            ;
            ...
            ? `<tr>
                    ...
                    <td>
                        <select class="code-block-category">
                            <option value="">${gettext("None")}</option>
                            ${enabledCategories
                .map(cat => `<option value="${cat}">${CATS[cat]?.[language] || cat}</option>`)
                .join("")}
                        </select>
                    </td>
                </tr>`
