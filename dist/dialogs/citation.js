import { Dialog, SelectionDataTable, addAlert, ensureCSS, escapeText, findTarget, setCheckableLabel } from "fwtoolkit";
...
export class CitationDialog {
    ...
    constructor(editor) {
        ...
        this.node = this.editor.currentView.state.selection
            .node;
        this.dialog = false;
        this.buttons = [];
    ...
    activatePlugins() {
        ...
        const pluginList = this.editor.citationDialogPlugins || defaultCitationPlugins;
        this.plugins = {};
        return Promise.all(pluginList.map(([app, plugin]) => {
            ...
            return Promise.all(Object.values(plugin).map(pluginExport => {
                if (typeof pluginExport === "function") {
                    ...
                    return (this.plugins[pluginExport.name].init() || Promise.resolve());
                }
                return Promise.resolve();
            }));
        })).then(() => undefined);
    ...
    createAllTableRows() {
        ...
        Object.keys(this.editor.mod.db.bibDB.db).forEach(id => {
            data.push(this.createTableRow(this.editor.mod.db.bibDB.db[id], Number.parseInt(id), "document"));
        });
    ...
    addToCitableItems(ids) {
        ...
        ids.forEach(id => {
            ...
            data.push(this.createTableRow(this.editor.mod.db.bibDB.db[id], id, "document"));
        });
    ...
    addToCitedItems(items) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            this.dialog.dialogEl
                .querySelector("#selected-cite-source-table .fw-data-table-body")
                ?.insertAdjacentHTML("beforeend", selectedCitationTemplate({
                id: item.id,
                db: item.db,
                title: item.title,
                author: item.author,
        ...
        this.dialog.dialogEl
            ...
            this.table.data.data.forEach((row) => {
                ...
                const [db, id] = String(rowId)
                    .split("-")
                    .map((val, index) => index ? Number.parseInt(val) : val);
    ...
    dialogSubmit() {
        const citeItems = Array.from(this.dialog.dialogEl.querySelectorAll("#selected-cite-source-table .fw-cite-parts-table")), references = citeItems.map(bibRef => {
            const deleteButton = bibRef.querySelector(".delete");
            const db = deleteButton.dataset.db;
