            ...
            buttons: [
                {
                    ...
                    click: () => {
                        const view = this.editor.currentView, state = view.state;
                        ...
                        else if (new RegExp(`^\\^({[${supChars}]*}|[${supChars}]?)$`).test(this.equation)) {
                            // The math input is pure superscript and
                            // can be converted to ordinary characters.
                            view.dispatch(state.tr.insertText(sup(this.equation.slice(1))));
                            this.dialog.close();
                            return;
                        }
                        else if (new RegExp(`^\\_({[${subChars}]*}|[${subChars}]?)$`).test(this.equation)) {
                            // The math input is pure subscript and
                            // can be converted to ordinary characters.
                            view.dispatch(state.tr.insertText(sub(this.equation.slice(1))));
                            this.dialog.close();
                            return;
                        }
                        else if (this.equationSelected &&
                        ...
                        view.dispatch(state.tr.replaceSelectionWith(nodeType.createAndFill({
