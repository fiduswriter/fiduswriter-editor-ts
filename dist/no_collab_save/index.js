...
export class NoCollabSave {
    ...
    async _handleVersionConflict() {
        ...
        try {
            const { json } = await this.editor.app.apiConnectors.document.getDocumentData(payload);
            const data = json;
            ...
            if (mappedSteps.length) {
                this.editor.view.dispatch(receiveTransaction(this.editor.view.state, mappedSteps, mappedSteps.map(() => "remote")).setMeta("remote", true));
            }
            ...
            this.editor.mod.comments.store.loadComments(data.doc.comments);
