...
import { citationRenderPlugin, clipboardPlugin, collabCaretsPlugin, commentsPlugin, figurePlugin, getFootnoteMarkerContents, linksPlugin, marginboxesPlugin, searchPlugin, selectionMenuPlugin, toolbarPlugin, trackPlugin, updateFootnoteMarker } from "../state_plugins/index.js";
...
export class ModFootnoteEditor {
    ...
    init() {
        const doc = this.schema.nodeFromJSON({ type: "doc", content: [] }), plugins = this.fnStatePlugins.map(plugin => {
            if (plugin[1]) {
        ...
        this.view = new EditorView(document.getElementById("footnote-box-container"), {
            ...
            dispatchTransaction: tr => {
                const remote = tr.getMeta("remote"), fromMain = tr.getMeta("fromMain");
    ...
    onTransaction(tr) {
        ...
        const mainTr = this.mod.editor.view.state.tr, mainState = this.mod.editor.view.state;
        if (tr.docChanged) {
            const fns = [
                ...new Set(tr.steps
                    .map((step, index) => {
