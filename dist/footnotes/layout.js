...
export class ModFootnoteLayout {
    ...
    updateDOM() {
        ...
        fastdomImpl.measure(() => {
            // DOM read phase
            const footnoteBoxes = document.querySelectorAll("#footnote-box-container .footnote-container"), referrers = getFootnoteMarkers(this.editor.view.state);
            if ((!referrers.length &&
                this.editor.mod.citations.citationType !==
                    "note") ||
            ...
            if (getComputedStyle(footnoteBoxContainer).display === "block") {
            ...
            else if (this.editor.mod.citations.citationType === "note") {
                ...
                let editorFootnoteIndex = 0, citationFootnoteIndex = 0, totalEditorOffset = totalOffset, totalCitationOffset = totalOffset;
                this.editor.view.state.doc.descendants((node, pos) => {
                    if (node.isInline &&
                        ...
                        if (node.type.name === "footnote") {
                            const footnoteBox = footnoteBoxes[editorFootnoteIndex], selector = `.footnote-container:nth-of-type(${editorFootnoteIndex + 1})`, footnoteBoxCoords = footnoteBox.getBoundingClientRect(), footnoteBoxHeight = footnoteBoxCoords.height, referrerTop = this.editor.view.coordsAtPos(pos).top;
                            editorFootnoteIndex++;
                            if (!referrerTop) {
                            ...
                            if (referrerTop > totalEditorOffset ||
                                totalEditorOffset <
                                    totalCitationOffset + topMargin) {
                                topMargin = Math.max(referrerTop - totalEditorOffset, totalCitationOffset -
                                    totalEditorOffset +
                                    topMargin);
                            ...
                            if (citationFootnotes.length > citationFootnoteIndex) {
                                const footnoteBox = citationFootnotes[citationFootnoteIndex], selector = ".footnote-citation:nth-of-type(" +
                                    (citationFootnoteIndex + 1) +
                                    ")", footnoteBoxCoords = footnoteBox.getBoundingClientRect(), footnoteBoxHeight = footnoteBoxCoords.height, referrerTop = this.editor.view.coordsAtPos(pos).top;
                                ...
                                if (referrerTop > totalCitationOffset ||
                                    totalCitationOffset <
                                        totalEditorOffset + topMargin) {
                                    topMargin = Math.max(referrerTop - totalCitationOffset, topMargin +
                                        totalEditorOffset -
                                        totalCitationOffset);
                                    footnotePlacementStyle += `${selector} {margin-top: ${topMargin}px;}\n`;
                                }
                ...
                referrers.forEach((referrer, index) => {
                    ...
                    const footnoteBoxCoords = footnoteBox.getBoundingClientRect(), footnoteBoxHeight = footnoteBoxCoords.height, referrerTop = this.editor.view.coordsAtPos(referrer.from).top, selector = `.footnote-container:nth-of-type(${index + 1})`;
                    if (!referrerTop) {
                        // footnote is not shown. Also hide the footnote from the editor.
                        footnotePlacementStyle += `${selector} {display: none;}\n`;
                        return;
            ...
            fastdomImpl.mutate(() => {
                //DOM write phase
                const placementStyleEl = document.getElementById("footnote-placement-style");
                if (placementStyleEl &&
