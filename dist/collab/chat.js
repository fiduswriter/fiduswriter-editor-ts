...
export class ModCollabChat {
    ...
    beep() {
        ...
        notification.play().catch(() => { } // Don't worry about it if the browser prohibits playing the sound
        );
    }
    ...
    async newMessage(message) {
        ...
        if (message.e2ee) {
            ...
            if (isE2EE) {
                try {
                    message.body = await E2EEEncryptor.decrypt(message.body, this.mod.editor.e2ee.key);
                }
                catch (error) {
                    console.error("E2EE: Failed to decrypt chat message", error);
                    message.body = gettext("[Encrypted message - decryption failed]");
                }
            }
            else {
                message.body = gettext("[Encrypted message - enter password to read]");
            }
        }
    ...
    async sendMessage(messageText) {
        ...
        if (isE2EE) {
            try {
                const encryptedBody = await E2EEEncryptor.encrypt(messageText, this.mod.editor.e2ee.key);
