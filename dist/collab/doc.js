...
import { getSelectionUpdate, removeCollaboratorSelection, updateCollaboratorSelection } from "../state_plugins/index.js";
import { Merge } from "./merge/index.js";
export class ModCollabDoc {
    mod;
    merge;
    unconfirmedDiffs;
    confirmStepsRequestCounter;
    awaitingDiffResponse;
    receiving;
    currentlyCheckingVersion;
    enableCheckVersion;
    footnoteRender;
    initialVersionConfirmed;
    initialDocLoaded;
    templateAdjustmentPending;
    sendNextDiffTimer;
    lastSelectionUpdateState;
    constructor(mod) {
        mod.doc = this;
        this.mod = mod;
    ...
    loadDocument(data) {
        ...
        if (!this.mod.editor.user.is_authenticated &&
            ...
            const sessionId = data.doc_info.session_id;
     ...
     * @private
     */
    async _loadE2EEDocument(doc, doc_info, isInitialLoad, urlFragmentPassword = "") {
    ...
    async _decryptAndLoadDoc(doc, key, salt, iterations, isInitialLoad, urlFragmentPassword) {
        ...
        if (!e2ee.snapshotManager) {
            const { E2EESnapshotManager } = await import("../e2ee/snapshot-manager.js");
            e2ee.snapshotManager = new E2EESnapshotManager(this.mod.editor);
        }
        ;
        e2ee.snapshotManager.setKey(key);
        ...
        if (doc.images) {
            ...
            for (const [id, image] of Object.entries(doc.images)) {
                ...
                if (typeof image.copyright === "string") {
                    try {
                        ;
                        decryptedImages[id].copyright = await E2EEEncryptor.decryptObject(image.copyright, key);
                    }
                    catch (_e) {
        ...
        if (isNewE2EEDocument && e2ee.snapshotManager) {
            ;
            e2ee.snapshotManager.sendInitialSnapshot(decryptedContent, decryptedComments, decryptedBibliography, this.mod.editor.docInfo.version);
        }
    ...
    sendToCollaborators() {
        ...
        this.mod.editor.ws.send(() => {
            if (this.awaitingDiffResponse ||
                ...
                this.disableDiffSending();
                const stepsToSend = sendableSteps(this.mod.editor.view.state), fnStepsToSend = sendableSteps(this.mod.editor.mod.footnotes.fnEditor.view
                    .state), commentUpdates = this.mod.editor.mod.comments.store
                    .unsentEvents(), bibliographyUpdates = this.mod.editor.mod.db.bibDB.unsentEvents(), imageUpdates = this.mod.editor.mod.db.imageDB.unsentEvents();
                if (!stepsToSend &&
                    !fnStepsToSend &&
    ...
    async _sendE2EEDiff() {
        ...
        this.disableDiffSending();
        const stepsToSend = sendableSteps(this.mod.editor.view.state), fnStepsToSend = sendableSteps(this.mod.editor.mod.footnotes.fnEditor.view.state), commentUpdates = this.mod.editor.mod.comments.store
            .unsentEvents(), bibliographyUpdates = this.mod.editor.mod.db.bibDB
            .unsentEvents(), imageUpdates = this.mod.editor.mod.db.imageDB
            .unsentEvents();
        if (!stepsToSend &&
            !fnStepsToSend &&
            !commentUpdates.length &&
