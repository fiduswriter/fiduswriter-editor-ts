...
import { addDeletionMarks, dispatchRemoveDiffdata, removeDiffFromJson, updateMarkData } from "../tools.js";
import { acceptChanges, addDeletedContentBack, copyChange, deleteContent, handleMarks, removeDecoration } from "./action.js";
...
function createDeletionHighlight(decos, from, to, state, options) {
    ...
    const inlineDeco = Decoration.inline(from, to, { class: "selected-dec" }, { type: "deletion-highlight" });
    const deco = [];
...
function getDecos(decos, merge, state) {
    ...
    decos = decos.remove(decos.find(undefined, undefined, spec => {
    ...
    if (!currentMarks.length) {
        const node = state.selection instanceof NodeSelection
        ...
        if (node && node.attrs.diffdata && node.attrs.diffdata.length > 0) {
            ...
            const dom = createDropUp(merge, markFound, linkMark), deco = Decoration.widget(startPos, dom);
            ...
            const foundMark = node.marks.find(mark => mark.type.name == "diffdata");
            const startPos = $head.pos; // position of block start.
            const dom = createDropUp(merge, foundMark, linkMark), deco = Decoration.widget(startPos, dom);
            const highlightDecos = createHiglightDecoration(foundMark.attrs.from, foundMark.attrs.to, state);
            highlightDecos.push(deco);
            return decos.add(state.doc, highlightDecos);
        }
        else if (linkMark) {
            const startPos = $head.pos;
            const dom = createLinkDropUp(linkMark), deco = Decoration.widget(startPos, dom);
            return decos.add(state.doc, [deco]);
        }
        return decos;
    }
    ...
    const dom = createDropUp(merge, diffMark, linkMark), deco = Decoration.widget(startPos, dom);
...
function deletionDecorations(decos, merge, doc, tr, deletionClass) {
    ...
    const changeset = new changeSet(tr).getChangeSet(), schema = merge.schema, commonDoc = merge.cpDoc, mapping = tr.mapping;
    changeset.changes.forEach(change => {
        if (change.deleted.length > 0) {
            ...
            let stepsInvolved = [];
            change.deleted.forEach(deletion => stepsInvolved.push(Number.parseInt(String(deletion.data.step))));
            ...
            const deletedContent = DOMSerializer.fromSchema(schema).serializeFragment(content);
...
function createDropUp(merge, diffMark, linkMark) {
    ...
    const requiredPx = 10;
    const tr = String(diffMark.attrs.diff).search("offline") != -1
        ? merge.offlineTr
        : merge.onlineTr;
    const trType = String(diffMark.attrs.diff).search("offline") != -1
        ? "offline"
        : "online";
    const opType = String(diffMark.attrs.diff).search("inserted") != -1
        ? "insertion"
        : "deletion";
...
export const diffPlugin = (options) => new Plugin({
    key,
    state: {
        init(config) {
            let baseTr = false;
            ...
            if (baseTr && doc) {
                decos = deletionDecorations(decos, options.merge, doc, baseTr, deletionClass);
            }
            return {
                baseTr: baseTr,
        ...
        apply(tr, _prev, oldState, state) {
            ...
            if (tr.getMeta("removeHighlight")) {
                newDecos = newDecos.remove(newDecos.find(undefined, undefined, spec => spec.type == "deletion-highlight"));
            ...
            if (tr.getMeta("decorationId")) {
                const decorationId = Number.parseInt(tr.getMeta("decorationId"));
                newDecos = newDecos.remove(newDecos.find(undefined, undefined, spec => spec.id == decorationId));
            }
            if (tr.getMeta("highlight")) {
                const data = tr.getMeta("highlight");
                const from = options.merge.mergedDocMap.map(Number.parseInt(data.from));
                const to = options.merge.mergedDocMap.map(Number.parseInt(data.to));
                if (from && to) {
                    newDecos = createDeletionHighlight(newDecos, from, to, state, options);
                }
            }
    ...
    props: {
        handleClick: (view, _pos, event) => {
            ...
            delFnToolTip.forEach(tooltip => (tooltip.childNodes[0].style.display =
                "none"));
            ...
            if (offlineDeleted) {
                ...
                options.merge.mergeView2.dispatch(options.merge.mergeView2.state.tr.setMeta("highlight", {
        ...
        return {
            update: view => {
                ...
                if (changePopUp) {
                    ...
                    if (dialogBox) {
                        if (bounding.right > dialogBox.offsetWidth ||
                            bounding.right >
                                (window.innerWidth ||
