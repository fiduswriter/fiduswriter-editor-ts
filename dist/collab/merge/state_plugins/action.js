...
export const copyChange = (view, from, to) => {
    ...
    try {
        ...
        showSystemMessage(gettext("Copy to clipboard failed. Please copy manually."));
    }
...
export const acceptChanges = (merge, mark, mergeView, originalView, tr) => {
    ...
    for (const stepIndex of steps) {
        ...
        if (mappedStep) {
            ...
            insertionTr.doc.type.schema, mappedStep.toJSON());
        }
...
export const deleteContent = (merge, view, diffMark, mappingNeeded = true) => {
    ...
    const rebasedFrom = rebasedMapping.map(diffMark.attrs.from), rebasedTo = rebasedMapping.map(diffMark.attrs.to);
