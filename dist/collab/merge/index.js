...
export class Merge {
    ...
    adjustDocument(data) {
        ...
        if (this.mod.editor.docInfo.version < data.doc.v &&
            ...
            unconfirmedTr.steps
                .slice()
                .reverse()
                .forEach((step, index) => rollbackTr.step(step.invert(unconfirmedTr.docs[unconfirmedTr.docs.length - index - 1])));
            // We reset to there being no local changes to send.
            this.mod.editor.view.dispatch(receiveTransaction(this.mod.editor.view.state, unconfirmedTr.steps, unconfirmedTr.steps.map(() => this.mod.editor.client_id)));
            this.mod.editor.view.dispatch(receiveTransaction(this.mod.editor.view.state, rollbackTr.steps, rollbackTr.steps.map(() => "remote")).setMeta("remote", true));
            ...
            this.mod.editor.view.dispatch(receiveTransaction(this.mod.editor.view.state, lostTr.steps, lostTr.steps.map(() => "remote")).setMeta("remote", true));
            ...
            if (conflicts.length > 0) {
                try {
                    editor = new MergeEditor(this.mod.editor, confirmedState.doc, unconfirmedTr.doc, toDoc, unconfirmedTr, lostTr, {
    ...
    autoMerge(unconfirmedTr, lostTr, data, selection) {
        ...
        const maps = new Mapping([]
            .concat(unconfirmedTr.mapping.maps
            .slice()
            .reverse()
            .map(map => map.invert()))
        ...
        unconfirmedTr.steps.forEach((step, index) => {
            ...
            if (mapped && !rebasedTr.maybeStep(mapped).failed) {
                ...
                maps.setMirror(unconfirmedTr.steps.length - index - 1, unconfirmedTr.steps.length +
                    lostTr.steps.length +
        ...
        if (WRITE_ROLES.includes(this.mod.editor.docInfo.access_rights) &&
            ...
            rebasedTrackedTr = trackedTransaction(rebasedTr, this.mod.editor.view.state, this.mod.editor.user, false, Date.now() - this.mod.editor.clientTimeAdjustment);
        }
        else {
            tracked = false;
        ...
        usedBibs.forEach(id => {
            if (!this.mod.editor.mod.db.bibDB.db[id] &&
                oldBibDB[id]) {
                ;
                    ...
                    this.mod.editor.mod.db.imageDB
                        ...
                        this.mod.editor.view.state.doc.descendants((node, pos) => {
                            if (node.type.name === "image" &&
                                ...
                                attrs["image"] = false;
                                const nodeType = this.mod.editor.currentView
        ...
        if (tracked) {
            showSystemMessage(gettext("The document was modified substantially by other users while you were offline. We have merged your changes in as tracked changes. You should verify that your edits still make sense."));
        }
        ;
        this.mod.editor.mod.footnotes.fnEditor.renderAllFootnotes();
    }
    getDocData(offlineDoc) {
        ...
        return {
            content: (pmArticle?.toJSON() || { type: "article" }),
            settings: getSettings(pmArticle),
            title: title,
            version: String(this.mod.editor.docInfo.version),
            comments: this.mod.editor.mod.comments.store
                .comments,
    ...
    handleMergeFailure(error, offlineDoc, _onlineDoc, mergeEditor = false) {
        ...
        // Prepare Export
        new ExportFidusFile(this.mod.editor.app, this.getDocData(offlineDoc), this.mod.editor.mod.db.bibDB, this.mod.editor.mod.db.imageDB);
