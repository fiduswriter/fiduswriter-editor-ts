...
class RecreateTransform {
    ...
    recreateChangeContentSteps() {
        ...
        let ops = [], afterStepJSON = JSON.parse(JSON.stringify(this.currentJSON));
    ...
    addSetNodeMarkup() {
        const fromDoc = this.schema.nodeFromJSON(this.currentJSON), toDoc = this.schema.nodeFromJSON(this.finalJSON);
        ...
        if (start !== null) {
            const fromNode = fromDoc.nodeAt(start), toNode = toDoc.nodeAt(start);
            try {
        ...
        while (textDiffs.length) {
            ...
            if (diff.added) {
                if (textDiffs[0]?.removed) {
                    ...
                    this.tr.replaceWith(offset, offset + (nextDiff?.value.length || 0), this.schema
                        .nodeFromJSON({ type: "text", text: diff.value })
                        .mark(marks));
                }
                else {
                    this.tr.insert(offset, this.schema
                        .nodeFromJSON({ type: "text", text: diff.value })
                        .mark(marks));
                }
                ...
                if (textDiffs[0]?.added) {
                    const nextDiff = textDiffs.shift();
                    this.tr.replaceWith(offset, offset + diff.value.length, this.schema
                        .nodeFromJSON({
                        type: "text",
                        text: nextDiff?.value || ""
                    })
                        .mark(marks));
                    offset += nextDiff?.value.length || 0;
                }
                else {
        ...
        while (oldSteps.length) {
            ...
            while (oldSteps.length && step.merge(oldSteps[0])) {
                ...
                if (step instanceof ReplaceStep &&
                    ...
                    step =
                        getReplaceStep(newTr.doc, addedStep.apply(step.apply(newTr.doc).doc).doc) || step.merge(addedStep);
                }
                else {
...
//# sourceMappingURL=recreate_transform.js.map