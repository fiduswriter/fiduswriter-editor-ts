...
import { Dialog, activateWait, deactivateWait, ensureCSS, faqDialog, showSystemMessage } from "fwtoolkit";
...
import { checkPresenceOfdiffdata, dispatchRemoveDiffdata, removeDiffFromJson, simplifyTransform } from "./tools.js";
export class MergeEditor {
    ...
    constructor(editor, cpDoc, offlineDoc, onlineDoc, offlineTr, onlineTr, db) {
        ...
        const schemaCached = this.schema.cached;
        schemaCached.bibDB = this.editor.mod.db.bibDB;
        schemaCached.imageDB = this.editor.mod.db.imageDB;
        // Carry over the encrypted-image decryption handler.
        schemaCached.decryptImage = this.editor.schema.cached
            ?.decryptImage;
    }
    init() {
        ...
        if (this.mergeView2.state.doc.attrs.tracked ||
            this.mergeView3.state.doc.attrs.tracked) {
            this.mergeView2.dispatch(this.mergeView2.state.tr
    ...
    checkResolution() {
        ...
        const offlineVersionDoc = this.mergeView3.state.doc, mergedVersionDoc = this.mergeView2.state.doc;
        ...
        if (this.mergeView2.dom.querySelector(".deletion-decoration") ||
    ...
    markChangesinDiffEditor(changeset, view, insertionClass, tr) {
        ...
        changeset.changes.forEach(change => {
            if (change.inserted.length > 0) {
                let stepsInvolved = [];
                change.inserted.forEach((insertion) => stepsInvolved.push(Number.parseInt(String(insertion.data.step))));
                ...
                tr.steps.forEach((step, index) => {
                    const stepAny = step;
                    if (stepAny.from >= change.fromB &&
                        stepAny.to <= change.toB &&
                    ...
                    else if (stepAny.from >= change.fromB &&
                        stepAny.to <= change.toB &&
        ...
        tr.steps.forEach((step, index) => {
            const stepAny = step;
            const from = tr.mapping.slice(index).map(stepAny.from);
            const to = tr.mapping.slice(index).map(stepAny.to, -1);
            if (step instanceof ReplaceStep &&
                ...
                !stepsTrackedByChangeset.includes(index)) {
                const stepOne = step.toJSON(), stepsInvolved = [];
                stepsInvolved.push(index);
                if (stepOne.mark &&
                    ["strong", "em", "underline", "link", "deletion"].includes(stepOne.mark.type)) {
                    tr.steps.forEach((trStep, trIndex) => {
                        // Check for other format changes within this range.
                        const trStepAny = trStep;
                        if ((trStep instanceof AddMarkStep ||
                ...
                dispatchTransaction: tr => {
                    ...
                    if (tr.docChanged) {
                        ...
                        if (!noTrack) {
                            ...
                            mapTr = trackedTransaction(mapTr, this.mergeView2.state, this.editor.user, !this.mergeView2.state.doc.attrs.tracked &&
                                !["write-tracked", "review-tracked"].includes(this.editor.docInfo.access_rights), Date.now() - this.editor.clientTimeAdjustment);
                        }
                    }
    ...
    renderCitation(view, elementId) {
        const settings = view.state.doc.attrs, bibliographyHeader = settings.bibliography_header[settings.language] ||
            BIBLIOGRAPHY_HEADERS[settings.language];
        const citRenderer = new RenderCitations(document.getElementById(elementId), settings.citationstyle, bibliographyHeader, this.editor.mod.db.bibDB, this.editor.app.csl);
        citRenderer.init();
    }
        ...
        try {
            ...
            this.offStepsNotTracked.forEach(markstep => {
                ...
                if (mappedMarkStep &&
                    ...
                    offlineRebaseMapping.setMirror(offlineTr.steps.length - stepIndex - 1, offlineTr.steps.length +
    ...
    unHideSections(view) {
        ...
        view.state.doc.forEach((child, offset, _index) => {
            if (child.attrs.optional) {
                ...
                unHideSectionTr.setNodeMarkup(offset, null, attrs);
            }
        });
        ...
        if (conflicts.length > 0) {
            ...
            tr.steps.forEach((step, index) => {
                ...
                if (mapped && !newTr.maybeStep(mapped).failed) {
                    ...
                    maps.setMirror(tr.steps.length - index - 1, tr.steps.length +
    ...
    updateDB(offlineDoc, db) {
        ...
        const footnoteFind = (node, usedImages, usedBibs) => {
            if (node.name === "citation") {
                node.attrs?.references?.forEach(ref => usedBibs.push(Number.parseInt(String(ref.id))));
            }
        ...
        usedImages = Array.from(new Set(usedImages));
        usedImages.forEach(id => {
            if (!this.editor.mod.db.imageDB.db[id] && oldImageDB[id]) {
                ...
                if (Object.keys(this.editor.app.imageDB.db).includes(String(id))) {
                    ;
                    ...
                    this.editor.mod.db.imageDB
                        .reUploadImage(id, String(oldImageDB[id].image), String(oldImageDB[id].title), oldImageDB[id].copyright)
                        .then(({ id, newId }) => {
                        ...
                        this.mergeView1.state.doc.descendants((node, pos) => {
                            if (node.type.name === "image" &&
                                ...
                                attrs["image"] = newId;
                                const nodeType = this.mergeView1.state.schema
                                    .nodes["image"];
                        ...
                        this.mergeView3.state.doc.descendants((node, pos) => {
                            if (node.type.name === "image" &&
                                ...
                                const attrs = Object.assign({}, node.attrs);
                                attrs["image"] = newId;
                                const nodeType = this.mergeView3.state.schema
                                    .nodes["image"];
