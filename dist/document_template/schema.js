...
export const serializeHelp = (content) => {
    const doc = { type: "doc", content }, pmNode = helpSchema.nodeFromJSON(doc), dom = helpSerializer.serializeNode(pmNode);
...
richtextMenuContent[1][0].content.push(blockTypeItem(type, {
    ...
    run(state, dispatch) {
        ...
        dispatch(state.tr.replaceSelectionWith(schema.nodeFromJSON(table)), false);
    }
}));
