import { Dialog, addAlert, addProgress, escapeText, get, shortFileTitle } from "fwtoolkit";
...
export class ModDocumentTemplate {
    ...
    addDocPartSettings() {
            ...
            content: hideableDocParts.map(docPart => ({
                ...
                action: (editor) => {
                    ...
                    editor.view.dispatch(editor.view.state.tr
                        .setNodeMarkup(offset, null, Object.assign({}, node.attrs, {
        ...
        const headerbarModel = this.editor.menu.headerbarModel;
        const settingsMenu = headerbarModel.content.find((menu) => menu.id === "settings");
        settingsMenu.content = settingsMenu.content.filter((item) => item.id !== "metadata");
        settingsMenu.content.unshift(metadataMenu);
    }
    addCopyAsMenuEntry() {
        const headerbarModel = this.editor.menu.headerbarModel;
        const fileMenu = headerbarModel.content.find((menu) => menu.id === "file");
        ...
        fileMenu.content.push({
            ...
            action: (editor) => {
                ...
                const canToggleE2EE = e2eeMode === "enabled" ||
                ...
                const selectTemplateDialog = new Dialog({
                    ...
                    body: `<p>
                        ...
                        <select class="fw-button fw-large fw-light">${Object.entries(editor.mod.documentTemplate.documentTemplates)
                        .map(([importId, dt]) => `<option value="${escapeText(importId)}">${escapeText(dt.title)}</option>`)
                        .join("")}</select>
                        ${e2eeHtml}`,
                    buttons: [
                        {
                            ...
                            click: () => {
                                ...
                                const targetE2EE = canToggleE2EE &&
                                    selectTemplateDialog.dialogEl.querySelector("#e2ee-copy-toggle")?.checked;
                                const doCopy = (e2eeOptions) => {
                                    const copier = new SaveCopy(editor.getDoc(), editor.mod.db.bibDB, editor.mod.db.imageDB, editor.user, selectTemplateDialog.dialogEl.querySelector("select").value, e2eeOptions, editor.app.apiConnectors.documentImport);
                                    copier
                                        .init()
                                        .then(({ docInfo }) => {
                                ...
                                if (isE2EE && !targetE2EE) {
                                    ...
                                    if (editor.e2ee && editor.e2ee.key) {
                                        ...
                                        enterPasswordDialog(async (password) => {
                                            try {
                                                const key = await E2EEKeyManager.deriveKey(password, new Uint8Array(atob(editor.docInfo
                                                    .e2ee_salt)
                                                    .split("")
                                                    .map(c => c.charCodeAt(0))), editor.docInfo
                                                    .e2ee_iterations ||
                                                    600000);
                                                doCopy({
                                                    sourceKey: key,
    ...
    addExportTemplateMenuEntries() {
        const headerbarModel = this.editor.menu.headerbarModel;
        const exportMenu = headerbarModel.content.find((menu) => menu.id === "export");
        ...
        const exportMenuEntries = this.exportTemplates.map(template => {
            if (template.file_type === "docx") {
                return {
                    ...
                    action: (editor) => {
                        import("@fiduswriter/document/exporter/docx/index").then(async ({ DOCXExporter }) => {
                            ...
                            const task = addProgress("info", `${title}: ${gettext("Exporting DOCX...")}`, { autoClose: 6000 });
                            const exporter = new DOCXExporter(doc, template.template_file, editor.mod.db.bibDB, editor.mod.db.imageDB, editor.app.csl);
                            exporter.progressCallback = (message, percentage) => task.update(percentage, message);
                            exporter.init();
                        });
                    },
                    disabled: (editor) => editor.app.isOffline()
                ...
                return {
                    ...
                    action: (editor) => {
                        import("@fiduswriter/document/exporter/odt/index").then(async ({ ODTExporter }) => {
                            const dialog = new TemplateExportDialog();
                            ...
                            const title = shortFileTitle(doc.title, doc.path || "");
                            const task = addProgress("info", `${title}: ${gettext("Exporting ODT...")}`, { autoClose: 6000 });
                            const exporter = new ODTExporter(doc, template.template_file, editor.mod.db.bibDB, editor.mod.db.imageDB, editor.app.csl);
                            exporter.progressCallback = (message, percentage) => task.update(percentage, message);
                            exporter.init();
                        });
                    },
    ...
    addDocumentStylesMenuEntries() {
        const headerbarModel = this.editor.menu.headerbarModel;
        const settingsMenu = headerbarModel.content.find((menu) => menu.id === "settings"), documentStyleMenu = settingsMenu.content.find((menu) => menu.id === "document_style");
    ...
    getCitationStyles() {
        return this.editor.app.csl
            .getStyles()
            .then((styles) => {
            this.citationStyles = styles;
        });
    }
    citationStyleFallbackTitle(citationstyle) {
    ...
    citationStyleFallbackId(citationstyle) {
        if (citationstyle.includes("note")) {
    ...
    addCitationStylesMenuEntries() {
        const headerbarModel = this.editor.menu.headerbarModel;
        const settingsMenu = headerbarModel.content.find((menu) => menu.id === "settings"), citationStyleMenu = settingsMenu.content.find((menu) => menu.id === "citation_style");
        if (citationStyleMenu) {
            citationStyleMenu.content =
                this.editor.view.state.doc.attrs.citationstyles.map((citationstyle) => ({
                    ...
                    action: (editor) => {
                        ...
                        editor.view.dispatch(editor.view.state.tr
                            .setDocAttribute("citationstyle", actualStyle)
                            .setMeta("settings", true));
                    },
                    selected: (editor) => {
