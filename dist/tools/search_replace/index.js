...
import { deselectSearchMatch, endSearch, getProtectedRanges, getSearchMatches, selectNextSearchMatch, selectPreviousSearchMatch, setSearchTerm } from "../../state_plugins/index.js";
...
export class SearchReplaceDialog {
    ...
    init() {
        let buttons = [
            {
                ...
                click: () => {
                    if (this.matches.selected !== false) {
                        if (this.matches.selected > 0 ||
                            !this.fnMatches.matches.length) {
                            this.editor.view.dispatch(selectPreviousSearchMatch(this.editor.view.state));
                        }
                        else {
                            this.editor.view.dispatch(deselectSearchMatch(this.editor.view.state));
                            this.editor.mod.footnotes.fnEditor.view.dispatch(selectPreviousSearchMatch(this.editor.mod.footnotes.fnEditor
                                .view.state));
                        }
                    }
                    else if (this.fnMatches.selected !== false) {
                        if (this.fnMatches.selected > 0 ||
                            ...
                            ;
                            this.editor.mod.footnotes.fnEditor.view.dispatch(deselectSearchMatch(this.editor.mod.footnotes.fnEditor
                                .view.state));
                            this.editor.view.dispatch(selectPreviousSearchMatch(this.editor.view.state));
                        }
                    }
                ...
                click: () => {
                    if (this.matches.selected !== false) {
                        if (this.matches.selected <
                            ...
                            !this.fnMatches.matches.length) {
                            this.editor.view.dispatch(selectNextSearchMatch(this.editor.view.state));
                        }
                        else {
                            this.editor.view.dispatch(deselectSearchMatch(this.editor.view.state));
                            this.editor.mod.footnotes.fnEditor.view.dispatch(selectNextSearchMatch(this.editor.mod.footnotes.fnEditor
                                .view.state));
                        }
                    }
                    else if (this.fnMatches.selected !== false) {
                        if (this.fnMatches.selected <
            ...
            buttons = buttons.concat([
                {
                    ...
                    click: () => {
                        if (this.matches.selected !== false) {
                            ...
                            const originalDoc = this.editor.mod
                    ...
                    click: () => {
                        if (this.matches.matches.length) {
                            ...
                            while (matches.length) {
                                ...
                                if (match &&
                                    !protectedRanges.find(({ from, to }) => !((match.from <= from &&
                                        match.to <= from) ||
                                        (match.from >= to &&
                                            match.to >= to)))) {
                        ...
                        if (this.fnMatches.matches.length) {
                            const tr = this.editor.mod.footnotes
                                .fnEditor.view.state.tr;
                            const originalDoc = this.editor.mod
                                .footnotes.fnEditor.view.state.doc;
        ...
        this.resultCountEl = this.dialog.dialogEl.querySelector(".search-result-count");
    ...
    setNoteState() {
        ...
        while (matches.length && !matchWithinPR) {
            ...
            if (match &&
                protectedRanges.find(({ from, to }) => !((match.from <= from && match.to <= from) ||
    ...
    updateResultCount() {
        const totalMatches = this.matches.matches.length + this.fnMatches.matches.length;
    ...
    onUpdate() {
        ...
        const selectedSearch = document.querySelector("#paper-editable .fw-search.fw-selected");
        if (selectedSearch) {
    ...
    search(term) {
        const setSearchTermResult = setSearchTerm(this.editor.view.state, term, this.editor.view === this.editor.currentView ? 0 : false, this);
        let { tr } = setSearchTermResult;
        const { matches, selected } = setSearchTermResult;
        const setSearchTermResultFn = setSearchTerm(this.editor.mod.footnotes.fnEditor.view.state, term, this.editor.mod.footnotes.fnEditor.view ===
            this.editor.currentView
            ? 0
            : false, this);
    ...
    endSearch() {
        ...
        this.editor.mod.footnotes.fnEditor.view.dispatch(endSearch(this.editor.mod.footnotes.fnEditor.view.state));
    }
}
