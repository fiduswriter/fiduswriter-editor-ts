...
export class SelectionMenuView {
    ...
    destroy() {
        ...
        menu.selectionMenuViews = menu.selectionMenuViews.filter(view => view !== this);
    }
    onclick(event) {
        ...
        if (target.matches(".editor-selection-menu > div:not(.fw-disabled), .editor-selection-menu > div:not(.fw-disabled) *")) {
            ...
            if (this.openedMenu !== false) {
                this.getMenu().selectionMenuModel.content[this.openedMenu].open = false;
            }
    ...
    getSelectionMenuHTML() {
        if (READ_ONLY_ROLES.includes(this.editor.docInfo.access_rights) ||
            this.editorView.state.selection.empty ||
            this.editor.mod.comments.store
                .commentDuringCreation ||
            this.editor.mod.comments.interactions.isCurrentlyEditing()) {
            return "<div></div>";
        }
        const selectionMenuTop = document.querySelector("#selection-menu").getBoundingClientRect().top, offset = this.editorView.coordsAtPos(this.editorView.state.selection.from).top - selectionMenuTop;
        return `<div style="margin-top: ${offset}px;">
            <div class="editor-selection-menu">
                ${this.getMenu().selectionMenuModel.content
            .map((menuItem, index) => `<div class="ui-buttonset${(menuItem.hidden && menuItem.hidden(this.editor)) || (menuItem.disabled && menuItem.disabled(this.editor)) ? " fw-disabled" : ""}">
