...
const tracksInSelection = (view) => {
    ...
    let tracks = false;
    const from = view.state.selection.from, to = view.state.selection.to;
    view.state.doc.nodesBetween(from, to, (node, pos) => {
        if (pos < from && !node.isInline) {
            ...
            if (["deletion", "format_change"].includes(mark.type.name) ||
    ...
    content: [
        {
            ...
            hidden: (editor) => editor.currentView.state.selection.$anchor.depth < 1,
        ...
        },
        {
            ...
            icon: "check-double",
            action: (editor) => acceptAll(editor.currentView, editor.currentView.state.selection.from, editor.currentView.state.selection.to),
        ...
        },
        {
            ...
            icon: "trash",
            action: (editor) => rejectAll(editor.currentView, editor.currentView.state.selection.from, editor.currentView.state.selection.to),
            disabled: (editor) => editor.docInfo.access_rights !== "write",
            hidden: (editor) => editor.currentView.state.selection.$anchor.depth < 1 ||
                !tracksInSelection(editor.currentView),
            order: 4
