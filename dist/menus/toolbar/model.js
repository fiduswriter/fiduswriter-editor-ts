...
import { CitationDialog, FigureDialog, LinkDialog, MathDialog, TableDialog } from "../../dialogs/index.js";
import { setBlockType } from "../../keymap.js";
import { checkProtectedInSelection } from "../../state_plugins/document_template.js";
import { getInlineReferenceState, setInlineReferenceState } from "../../state_plugins/inline_reference/index.js";
const BLOCK_LABELS = {
...
function currentSelection(editor) {
    return editor.currentView.state.selection;
}
...
function elementAvailable(editor, elementName) {
    ...
    editor.view.state.doc.forEach(docPart => {
        if (docPart.attrs.elements?.includes(elementName)) {
            elementInDocParts = true;
        }
    });
    const partElements = editor.view.state.selection.$anchor.node(1)?.attrs.elements ||
        editor.view.state.selection.$anchor.node(1)?.type.spec.attrs
            ?.elements?.default;
    return (partElements?.includes(elementName) ||
...
export function elementDisabled(editor, elementName) {
    if (editor.currentView === editor.view) {
        // main editor
        const anchorDocPart = editor.currentView.state.selection.$anchor.node(1), headDocPart = editor.currentView.state.selection.$head.node(1);
        const partElements = anchorDocPart?.attrs.elements ||
            anchorDocPart?.type.spec.attrs?.elements?.default;
        ...
        // footnote editor
        const anchorFootnote = editor.currentView.state.selection.$anchor.node(1), headFootnote = editor.currentView.state.selection.$head.node(1);
...
function markAvailable(editor, markName) {
    ...
    editor.view.state.doc.forEach(docPart => {
        if (docPart.attrs.marks?.includes(markName)) {
            markInDocParts = true;
        }
    });
    return (editor.view.state.doc.attrs.footnote_marks.includes(markName) ||
        markInDocParts);
}
    ...
    content: [
        ...
        },
        {
            ...
            show: (editor) => {
                ...
                else if (editor.currentView.state.selection.$anchor.node(1) &&
                ...
                else if (editor.currentView.state.selection.$anchor.depth === 0 &&
        ...
        },
        {
            ...
            show: (editor) => {
                const selection = currentSelection(editor);
                if (editor.currentView.state.selection.$anchor.node(1) &&
                    !editor.view.state.selection.$anchor.node(1).attrs.elements &&
                    !editor.view.state.schema.nodes.richtext_part?.spec?.attrs?.elements?.default) {
                    return "";
                }
                ...
                const startElement = editor.currentView.state.selection.$anchor.parent, endElement = editor.currentView.state.selection.$head.parent;
                if (!startElement || !endElement) {
                    ...
                    editor.currentView.state.doc.nodesBetween(editor.currentView.state.selection.from, editor.currentView.state.selection.to, node => {
                        if (node.isTextblock) {
            ...
            disabled: (editor) => READ_ONLY_ROLES.includes(editor.docInfo.access_rights) ||
                ...
                !(editor.currentView.state.selection.$anchor.node(1).attrs
                    .elements ||
                    editor.currentView.state.schema.nodes.richtext_part?.spec?.attrs?.elements?.default) ||
        ...
        },
        {
            ...
            selected: (editor) => {
                ...
                if (storedMarks?.some((mark) => mark.type.name === "strong") ||
        ...
        },
        {
            ...
            selected: (editor) => {
                const storedMarks = editor.currentView.state.storedMarks;
                if (storedMarks?.some((mark) => mark.type.name === "em") ||
                    editor.currentView.state.selection.$head
                        .marks()
        ...
        },
        {
            ...
            selected: (editor) => {
                const storedMarks = editor.currentView.state.storedMarks;
                if (storedMarks?.some((mark) => mark.type.name === "underline") ||
                    editor.currentView.state.selection.$head
                        .marks()
        ...
        },
        {
            ...
            selected: (editor) => {
                const storedMarks = editor.currentView.state.storedMarks;
                if (storedMarks?.some((mark) => mark.type.name === "sup") ||
                    editor.currentView.state.selection.$head
                        .marks()
        ...
        },
        {
            ...
            selected: (editor) => {
                const storedMarks = editor.currentView.state.storedMarks;
                if (storedMarks?.some((mark) => mark.type.name === "sub") ||
                    editor.currentView.state.selection.$head
                        .marks()
        ...
        },
        {
            ...
            selected: (editor) => {
                const depth = editor.currentView.state.selection.$head.sharedDepth(editor.currentView.state.selection.anchor);
        ...
        },
        {
            ...
            selected: (editor) => editor.currentView.state.selection.$head
                .marks()
        ...
        },
        {
            ...
            icon: "book",
            action: (editor) => {
                const inlineState = getInlineReferenceState(editor.currentView.state);
                if (inlineState?.active && inlineState.isEdit) {
                    editor.currentView.dispatch(setInlineReferenceState(editor.currentView.state.tr, {
                        action: "openDialog"
                    }));
                }
            ...
            disabled: (editor) => {
                if (READ_ONLY_ROLES.includes(editor.docInfo.access_rights) ||
                    ...
                    !["text", "node"].includes(currentSelection(editor).jsonID) ||
                    (currentSelection(editor).jsonID === "node" &&
        ...
        },
        {
            ...
            icon: "minus",
            action: (editor) => {
                const view = editor.currentView, state = view.state;
                view.dispatch(state.tr.replaceSelectionWith(state.schema.node("horizontal_rule")));
            },
