...
import { addAlert, avatarTemplate, cleanPath, escapeText, findTarget } from "fwtoolkit";
export class HeaderbarView {
    ...
    constructor(editorView, options) {
        ...
        if (!headerbar?.firstElementChild) {
            throw new Error("Headerbar element not found.");
        }
        this.headerEl = headerbar.firstElementChild;
        this.listeners = {};
        const model = this.options.editor.menu.headerbarModel;
        this.removeUnavailable(model);
        this.addMissingIds(model);
        this.bindEvents();
        this.update();
        this.parentChain = [];
        this.openMenu = null;
    ...
    onclick(event) {
        ...
        const model = this.editor.menu.headerbarModel;
        if (target.closest("div#close-document-top a i.fa-times")) {
            ...
            if (this.editor.app.isOffline()) {
                ...
                addAlert("info", gettext("You cannot close a document when you're offline."));
            }
        }
        else if (target.closest("#headerbar #header-navigation .fw-pulldown-item")) {
            // A header nav menu item was clicked. Now we just need to find
            // which one and execute the corresponding action.
            const item = target.closest("#headerbar #header-navigation .fw-pulldown-item");
            const searchPath = [];
            let seekItem = item;
            while (seekItem?.closest("li")) {
                ...
                seekItem = seekItem?.parentElement ?? null;
            }
            seekItem = seekItem?.closest("div.header-menu") ?? null;
            let menuNumber = 0;
            while (seekItem?.previousElementSibling) {
                menuNumber++;
                seekItem = seekItem.previousElementSibling;
            }
            const menu = model.content[menuNumber];
            let menuItem = menu;
            while (searchPath.length) {
                const index = searchPath.pop();
        ...
        else if (target.closest("#headerbar #header-navigation .header-nav-item:not(.fw-disabled)")) {
            // A menu has been clicked, lets find out which one.
            const item = target.closest("#headerbar #header-navigation .header-nav-item:not(.fw-disabled)");
            let menuNumber = 0;
            let seekItem = item?.parentElement ?? null;
            while (seekItem?.previousElementSibling) {
                menuNumber++;
                seekItem = seekItem.previousElementSibling;
            }
            model.content.forEach(menu => {
                if (menu.open) {
                    menu.open = false;
                    this.openMenu = null;
            ...
            model.content[menuNumber].open = true;
            this.openMenu = model.content[menuNumber];
                    ...
                    if (isMenuItemInParentChain) {
                        ...
                        for (let index = this.parentChain.length - 2; index >= 0; index--) {
                            if (this.parentChain[index].content.find(menu => menu.id === menuItem.id)) {
                                ...
                                this.closeOtherMenu(this.parentChain[this.parentChain.length - 1], menuItem);
                                this.parentChain.push(menuItem);
    ...
    closeOtherMenu(menu, currentMenuItem) {
        menu.content.forEach(menuItem => {
            if (menuItem.type === "menu" && menuItem.open) {
                if (!this.parentChain.includes(menuItem) &&
    ...
    onKeydown(event) {
        ...
        if (this.openMenu) {
            if (["ArrowDown", "ArrowUp", "ArrowLeft", "ArrowRight"].includes(name)) {
                ...
                this.executeMenuItem(menuItem, this.parentChain[this.parentChain.length - 1]);
                return;
            }
            else if (name === "Escape") {
                event.preventDefault();
    ...
    changeCursorMenuItem(name) {
        ...
        if (!this.cursorMenuItem) {
            ...
            if (["ArrowDown", "ArrowUp"].includes(name)) {
                let index = this.parentChain[this.parentChain.length - 1].content.indexOf(this.cursorMenuItem);
                if (name === "ArrowDown") {
                    if (index <
                ...
                if (this.parentChain.length > 1) {
                    ...
                    this.closeAllMenu(this.parentChain[this.parentChain.length - 1]);
                }
                else {
                    const currentMenuIndex = model.content.findIndex(menu => menu.open);
                    ...
                    this.openMenu = model.content[newMenuIndex];
                    this.openMenu.open = true;
                ...
                if (this.cursorMenuItem.type === "menu") {
                    ...
                    const currentMenuIndex = model.content.findIndex(menu => menu.open);
                    const newMenuIndex = currentMenuIndex === model.content.length - 1
                        ? 0
                        : currentMenuIndex + 1;
                    if (this.openMenu) {
                        this.openMenu.open = false;
    ...
    checkKeys(event, menu, nameKey) {
        menu.content.forEach(menuItem => {
            if ("keys" in menuItem && menuItem.keys === nameKey) {
                event.preventDefault();
    ...
    getHeaderHTML() {
        ...
        if (!editorOnlyMode) {
            ...
            if (this.editor.docInfo.token) {
                ...
                const folderPath = this.editor.docInfo.path.slice(0, this.editor.docInfo.path.lastIndexOf("/"));
                exitUrl =
                    !folderPath.length &&
        ...
        return model.content
            .map(menu => `
                <div class="header-menu">
                    <span class="header-nav-item${menu.disabled?.(this.editor) ? " fw-disabled" : ""}"
    ...
    getMenuHTML(menu) {
        const title = typeof menu.title === "function"
    ...
    getActionMenuItemHTML(menuItem) {
        return `<span class="fw-pulldown-item${menuItem.selected?.(this.editor)
            ? " fw-selected"
            : ""}${menuItem.disabled?.(this.editor) ? " fw-disabled" : ""}${menuItem === this.cursorMenuItem ? " fw-cursor" : ""}"
        role="menuitem"
        ${menuItem.disabled?.(this.editor) ? 'aria-disabled="true"' : ""}
        ${menuItem.selected?.(this.editor) ? 'aria-checked="true"' : ""}
    ...
    getMenuMenuItemHTML(menuItem) {
        return `<span class="fw-pulldown-item${menuItem.selected?.(this.editor)
            ? " fw-selected"
            : ""}${menuItem.disabled?.(this.editor) ? " fw-disabled" : ""}${menuItem === this.cursorMenuItem ? " fw-cursor" : ""}" ${menuItem.tooltip ? `title="${menuItem.tooltip}" aria-label="${menuItem.tooltip}"` : ""}>
