...
import { Dialog, WebSocketConnector, activateWait, addAlert, deactivateWait, ensureCSS, showSystemMessage, whenReady } from "fwtoolkit";
...
import { createPasswordDialog } from "fwtoolkit/e2ee/password-dialog";
...
import { codeBlockMenuModel, figureMenuModel, figureWidthMenuModel, headerbarModel, imageMenuModel, navigatorFilterModel, orderedListMenuModel, selectionMenuModel, tableMenuModel, toolbarModel } from "./menus/index.js";
...
export class Editor {
    ...
    plugins;
    noCollabSave;
    constructor({ app, user, mount }, path, idString, editorPlugins = defaultEditorPlugins, citationDialogPlugins = null) {
        this.app = app;
        this.mount = mount;
    ...
    init() {
        ...
        if (this.docInfo.hasOwnProperty("templateId")) {
            ...
            if (shouldEncrypt) {
                ...
                initPromises.push(this.app.apiConnectors.document
                    .createDocument({
        ...
        if (uuid4Pattern.test(this.docInfo.id)) {
            ...
            initPromises.push(this.app.apiConnectors.document
                .validateShareToken(this.docInfo.token)
                .then(({ json, status }) => {
        ...
        return Promise.all(initPromises)
            ...
            const editorSaveMode = this.app.settings.EDITOR_SAVE_MODE ?? "collaborative";
            ...
            const stylesPromise = documentApi
                .getDocumentStyles(stylesPayload)
                .catch(() => {
                return { json: {} };
            });
            const docDataPromise = documentApi
                .getDocumentData(stylesPayload)
                .catch(error => {
                ...
                if (error.message === "Invalid or expired share link") {
                    ...
                    deactivateWait();
                    const requestAccessDialog = new RequestAccessDialog(this.docInfo.id, "", documentApi, Boolean(this.docInfo.token));
                                ...
                                buttons: [
                                    {
                                        ...
                                        click: () => {
                                            ...
                                            new ExportFidusFile(this.app, doc, this.mod.db.bibDB, this.mod.db.imageDB, false);
                                        }
                                    },
                                    {
            ...
            if (editorSaveMode === "direct") {
                if (!this.noCollabSave) {
                    this.noCollabSave = new NoCollabSave(this, this.app.settings.SAVE_POLLING);
                }
    ...
    handleAccessRightModification() {
        ...
        new ExportFidusFile(this.app, this.getDoc({ use_current_view: true }), this.mod.db.bibDB, this.mod.db.imageDB);
    ...
    async _createE2EEDocument() {
        ...
        if (hasPassphraseKeys) {
            ...
            if (!PassphraseManager.hasKeysInSession()) {
                ...
                if (result.action === "unlock" && result.passphrase) {
                    ...
                    // Recovery flow
                    const { recoverWithKeyDialog } = await import("fwtoolkit/e2ee/passphrase-dialog");
                    ...
                    if (recoverResult) {
                        try {
                            ...
                            const { showRecoveryKeyDialog } = await import("fwtoolkit/e2ee/passphrase-dialog");
        ...
        if (status === 403) {
            addAlert("error", createData.error || gettext("E2EE is not enabled on this server."));
            window.location.href = "/";
            return;
        }
    ...
    async activateFidusPlugins() {
        ...
        // Add plugins.
        this.plugins = {};
        await Promise.all(this.editorPlugins.map(([app, plugin]) => {
            if (!this.app.settings.APPS.includes(app)) {
                return Promise.resolve();
            }
            return Promise.all(Object.values(plugin).map(pluginExport => {
                if (typeof pluginExport === "function") {
                    const name = pluginExport.name;
                    this.plugins[name] = new pluginExport(this);
                    return (this.plugins[name].init() ||
                        Promise.resolve());
                }
                return Promise.resolve();
            }));
    ...
    scrollIdIntoView(id) {
        ...
        if (foundPos === false) {
            const fnView = this.mod.footnotes.fnEditor.view;
            fnView.state.doc.descendants((node, pos) => {
                if (foundPos !== false) {
                    return;
                }
                else if ((node.type.groups.includes("heading") ||
