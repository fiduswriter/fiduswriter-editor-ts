...
export class FidusWriterPasteHandler extends GeneralPasteHandler {
    ...
    verifyCitationNode(node) {
        try {
            const bibs = JSON.parse(node.dataset.bibs || "{}"), references = JSON.parse(node.dataset.references || "[]"), bibDB = this.editor.mod.db?.bibDB, idTranslations = {};
            if (!bibDB) {
                return node;
            }
    ...
    cleanDOM() {
        ...
        const clipboardArtifacts = this.dom.querySelectorAll(".fiduswriter-clipboard-bibliography, .fiduswriter-clipboard-footnotes");
        clipboardArtifacts.forEach(el => el.parentElement?.removeChild(el));
    }
}
//# sourceMappingURL=fidus_writer.js.map