...
class ClipboardDOMSerializer {
    ...
    renderCitations(domFragment) {
        ...
        const citRenderer = new RenderCitations(domFragment, settings.citationstyle, bibliographyHeader, this.editor.mod.db.bibDB, this.editor.app.csl, true // synchronous. Should work as the editor has used the same style previously.
        );
        if (citRenderer.init()) {
            const fm = citRenderer.fm;
    ...
    renderFootnotes(domFragment, citationFormatter) {
        ...
        footnotes.forEach((footnote, index) => {
            ...
            if (newFootnote.firstElementChild &&
    ...
    addFigureNumbers(domFragment) {
        domFragment
            .querySelectorAll("figure[data-category='figure'] figcaption span.label")
            .forEach((el, index) => {
            ;
            el.innerHTML +=
                " " + (index + 1) + ": ";
        ...
        domFragment
            .querySelectorAll("figure[data-category='table'] figcaption span.label")
            .forEach((el, index) => {
            ;
            el.innerHTML +=
                " " + (index + 1) + ": ";
    ...
    static fromSchema(schema, editor) {
        return new ClipboardDOMSerializer(DOMSerializer.nodesFromSchema(schema), DOMSerializer.marksFromSchema(schema), editor);
    }
}
export const docClipboardSerializer = (editor) => ClipboardDOMSerializer.fromSchema(createDocCopySchema(editor.schema), editor);
