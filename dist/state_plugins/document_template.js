...
export function addDeletedPartWidget(dom, view, getPos) {
    ...
    dom.insertAdjacentHTML("beforeend", '<div class="remove-doc-part"><i class="fa-solid fa-trash-alt"></i></div>');
    ...
    removeButton.addEventListener("click", () => {
        ...
        const from = pos, to = from + view.state.doc.nodeAt(from).nodeSize, tr = view.state.tr;
        tr.delete(from, to);
...
export function getAllowedElementsAndMarks(state) {
    ...
    const allowedElements = docPart.attrs.elements
        ? docPart.attrs.elements.concat("table_caption", "table_body", "table_row", "table_cell", "table_header", "list_item", "text")
        : false;
    const allowedMarks = docPart.attrs.marks
        ? docPart.attrs.marks.concat("insertion", "deletion", "comment", "anchor")
        : false;
...
export function checkProtectedInSelection(state) {
    ...
    // within a selection
    const anchorDocPart = state.selection.$anchor.node(1), headDocPart = state.selection.$head.node(1);
    ...
    if (["start", "header"].includes(anchorDocPart.attrs.locking) ||
        ...
        const protectedRanges = getProtectedRanges(state), start = state.selection.from, end = state.selection.to;
        if (protectedRanges.find(({ from, to }) => !((start <= from && end <= from) ||
            (start >= to && end >= to)))) {
            return true;
        }
    }
...
export const documentTemplatePlugin = (options) => new Plugin({
    key,
    state: {
        init(_config, state) {
            if (options.editor.docInfo.access_rights === "write") {
                ;
                this.spec.props.nodeViews["richtext_part"] = (node, view, getPos) => new PartView(node, view, getPos);
                this.spec.props.nodeViews["heading_part"] = (node, view, getPos) => new PartView(node, view, getPos);
                this.spec.props.nodeViews["table_part"] = (node, view, getPos) => new PartView(node, view, getPos);
            ...
            state.doc.forEach((node, pos) => {
                ...
                if (node.attrs.locking === "fixed") {
                    ...
                    if (node.content.size >= protectionSize &&
                        initialFragment.eq(node.slice(0, protectionSize).content)) {
        ...
        apply(tr, _prev, oldState, _state) {
            let { protectedRanges } = key.getState(oldState);
    ...
    filterTransaction: (tr, state) => {
        ...
        changingRanges.forEach(range => state.doc.nodesBetween(range.start, range.end, (node, pos, parent, _index) => {
            const elements = allowedElements;
            const marks = allowedMarks;
            if (parent === tr.doc) {
                allowedElements = node.attrs.elements
                    ? node.attrs.elements.concat("table_caption", "table_body", "table_row", "table_cell", "table_header", "list_item", "text")
                    : false;
...
//# sourceMappingURL=document_template.js.map