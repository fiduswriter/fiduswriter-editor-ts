...
const moveComment = (doc, id, pos) => {
    ...
    while (doc.resolve(posFrom).depth > 1 &&
    ...
    if (doc.resolve(posFrom).depth === 1) {
        posFrom = posTo;
        posTo++;
        while (doc.resolve(posTo).depth > 1 &&
            !doc.textBetween(posFrom, posTo).length) {
            posTo++;
        }
...
export const addCommentDuringCreationDecoration = (state, tr) => {
    ...
    const commentDuringCreationDeco = decos.find(undefined, undefined, spec => spec === commentDuringCreationDecoSpec)[0];
...
export const commentsPlugin = (options) => new Plugin({
    key,
    state: {
        init() {
            return {
    ...
    appendTransaction: (trs, _oldState, state) => {
        ...
        trs.forEach(tr => {
            ...
            tr.steps.forEach((step, index) => {
                ...
                if (stepData.jsonID === "replace" ||
                    stepData.jsonID === "replaceAround") {
                    if (stepData.from !== stepData.to) {
                        ;
                        tr.docs[index].nodesBetween(stepData.from, stepData.to, (node) => {
                            node.marks
                                .filter(mark => mark.type.name === "comment" &&
                                mark.attrs.id)
                                .map(mark => mark.attrs.id)
                                .forEach(commentId => {
                                if (!Object.prototype.hasOwnProperty.call(deletedComments, commentId)) {
        ...
        let steps = [];
        let tr = state.tr;
        Object.entries(deletedComments).forEach(([_id, pos]) => {
        ...
        if (!steps.every(step => step)) {
            ...
            Object.entries(deletedComments).forEach(([_id, pos]) => {
                const id = Number.parseInt(_id);
                steps.push(moveComment(tr.doc, id, pos + 1));
            });
