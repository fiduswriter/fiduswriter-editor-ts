...
import { buildBibliographyList, buildCrossRefList, createInlineReferenceDropUp, filterBibliography, filterCrossRefs } from "./dropup.js";
 ...
 */
function createInlineReferenceWidget(editor, pluginState, pluginKey) {
        ...
        switch (event.key) {
            ...
            case "Enter":
                event.preventDefault();
                {
                    ...
                    const selectedIndex = Math.max(0, Math.min(state?.selectedIndex ?? -1, totalEnter - 1));
                    if (state?.listActive &&
                        totalEnter > 0 &&
...
export const inlineReferencePlugin = (options) => {
    ...
    const enabled = appConfig.config?.user?.preferences?.inline_references === true &&
        ...
        props: {
            ...
            handleDOMEvents: {
                mousedown(view, event) {
                    ...
                    const xrefTarget = !citTarget && target?.closest(".cross-reference");
                ...
                click(view, event) {
                    ...
                    if (pluginState?.active) {
                        const target = event.target;
                        const closest = target?.closest(".inline-reference-widget");
                        if (closest) {
