 ...
 */
export function createInlineReferenceDropUp(citationMatches, selectedIndex, onSelect, crossRefMatches = []) {
...
export function buildCrossRefList(editor) {
    ...
    if (editor.mod.footnotes?.fnEditor?.view?.state) {
        targets.push(...getInternalTargets(editor.mod.footnotes.fnEditor.view.state, language, "foot"));
    }
    return targets;
}
/**
