...
import { key, selectedChangeBlockSpec, selectedChangeFormatSpec, selectedDeletionSpec, selectedInsertionSpec } from "./plugin.js";
export function getSelectedChanges(state) {
    ...
    const insertion = decos.find(undefined, undefined, spec => spec === selectedInsertionSpec)[0], deletion = decos.find(undefined, undefined, spec => spec === selectedDeletionSpec)[0], format_change = decos.find(undefined, undefined, spec => spec === selectedChangeFormatSpec)[0], block_change = decos.find(undefined, undefined, spec => spec === selectedChangeBlockSpec)[0];
    return { insertion, deletion, format_change, block_change };
}
export function setSelectedChanges(state, type, pos) {
    const tr = state.tr, node = tr.doc.nodeAt(pos), mark = node?.attrs.track
    ...
    decos = decos.add(tr.doc, [
        decoType(selectedChange.from, selectedChange.to, {
            class: `selected-${type}`
        }, spec)
    ]);
