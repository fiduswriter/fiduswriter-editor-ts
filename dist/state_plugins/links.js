...
import { randomFigureId, randomHeadingId, randomListId, randomTableId } from "@fiduswriter/document/schema/common/index";
...
const copyLink = (href) => {
    ...
    try {
        ...
        addAlert("info", gettext("Copy to clipboard failed. Please copy manually."));
    }
};
...
export const linksPlugin = (options) => {
    ...
    function getUrl(state, oldState, oldUrl) {
        const id = state.selection.$head.parent.attrs.id, mark = state.selection.$head
            .marks()
    ...
    function getDecos(state, selectedIndex = -1) {
        ...
        if (inlineNode) {
            ...
            while (index < $head.parent.childCount - 1 &&
        ...
        const dom = createDropUp(linkMark, anchorMark, crossRef, $head, citation, selectedIndex), deco = Decoration.widget(startPos, dom);
        return DecorationSet.create(state.doc, [deco]);
    }
    function createDropUp(linkMark, anchorMark, crossRef, $head, citation, selectedIndex = -1) {
        const dropUp = document.createElement("span"), writeAccess = editor.docInfo.access_rights === "write" ? true : false, editAccess = ["write", "write-tracked", "review-tracked"].includes(editor.docInfo.access_rights)
            ? true
            : false;
...
${citation
        ...
        setupAction(".remove-anchor", () => {
            editor.view.dispatch(editor.view.state.tr.removeMark($head.start(), $head.end(), anchorMark));
        });
        ...
        setupAction(".remove-crossRef", () => {
            editor.view.dispatch(editor.view.state.tr.delete($head.pos - 1, $head.pos));
        });
    ...
    return new Plugin({
        ...
        state: {
            ...
            apply(tr, _prev, oldState, state) {
                let { url, decos, linkMark, anchorMark, crossReference, citation, selectedIndex } = key.getState(oldState);
                ...
                if (!tr.getMeta("remote")) {
                    ...
                    tr.steps.forEach((step, index) => {
                        const stepWithRange = step;
                        ranges.push([stepWithRange.from, stepWithRange.to]);
                        tr.docs[index].nodesBetween(stepWithRange.from, stepWithRange.to, node => {
                            if (node.type.groups.includes("heading") ||
        ...
        appendTransaction: (trs, oldState, newState) => {
            ...
            const internalTargets = getInternalTargets(newState, language, oldState.schema === options.editor.view.state.schema
                ? "main"
                : "foot").concat(getInternalTargets(otherState, language, oldState.schema === options.editor.view.state.schema
                ? "foot"
                : "main"));
            ...
            newState.doc.descendants((node, pos) => {
                if (node.type.groups.includes("heading") ||
                    ...
                    if (ids.includes(node.attrs.id) || !node.attrs.id) {
                        ...
                        const randomIdGenerator = node.type.groups.includes("heading")
                            ? randomHeadingId
                            : node.type.name === "figure"
                                ? randomFigureId
                ...
                else if (node.type.name === "cross_reference" &&
                    !internalTargets.find(it => it.id === node.attrs.id &&
                ...
                node.marks.forEach(mark => {
                    if (mark.type.name === "link" &&
                        mark.attrs.href[0] === "#" &&
                        !internalTargets.find(it => it.id === mark.attrs.href.slice(1) &&
                            it.text === node.attrs.title)) {
            ...
            if (foundAnchorWithoutId) {
                ...
                newTr.step(new RemoveMarkStep(0, newState.doc.content.size, markType));
            }
            return newTr;
        },
        props: {
            ...
            handleKeyDown(view, event) {
                ...
                const { linkMark, anchorMark, crossReference, citation, selectedIndex } = pluginState;
