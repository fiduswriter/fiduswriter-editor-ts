...
export const setSearchTerm = (state, term, selected = false, listener = false) => {
    const matches = findMatches(state.doc, term), decos = matchesToDecos(state.doc, matches, selected);
    selected =
...
export const endSearch = (state) => state.tr.setMeta(key, {
    term: "",
...
export const deselectSearchMatch = (state) => {
    ...
    const selected = false, decos = matchesToDecos(state.doc, matches, selected);
    return state.tr.setMeta(key, {
        term,
        decos,
        matches,
...
export const searchPlugin = (_options) => new Plugin({
    key,
    state: {
        init() {
            return {
