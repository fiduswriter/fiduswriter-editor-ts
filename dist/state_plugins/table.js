...
export const tablePlugin = (options) => new Plugin({
    key,
    state: {
        init(_config, _state) {
            const self = this;
            if (WRITE_ROLES.includes(options.editor.docInfo.access_rights)) {
                self.spec.props.nodeViews["table"] = (node, view, getPos) => new TableView(node, view, getPos, options);
            }
            self.spec.props.nodeViews["table_caption"] = (node, view, getPos) => new TableCaptionView(node, view, getPos, options);
            return {};
        },
        apply(_tr, prev) {
            return prev;
