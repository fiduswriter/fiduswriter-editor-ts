...
export const settingsPlugin = (options) => {
    const fixSettings = (settings) => {
        ...
        Object.keys(settings).forEach(key => {
            ...
            switch (key) {
                case "documentstyle":
                    if (!options.editor.mod.documentTemplate.documentStyles.find((d) => d.slug === value) &&
                        options.editor.mod.documentTemplate
                    ...
                    break;
                case "citationstyle":
                    if (!settings.citationstyles.includes(value) &&
    ...
    const updateDocStyleCSS = (docStyleId) => {
        ...
        docStyle.documentstylefile_set.forEach(([url, filename]) => (docStyleCSS = docStyleCSS.replace(new RegExp(filename, "g"), url)));
        ...
        view(view) {
            ...
            return {
                update: (view, _prevState) => {
                    updateSettings(view.state.doc.attrs, _prevState.doc.attrs);
