 ...
 */
export const contributorInputPlugin = (options) => new Plugin({
    key,
    state: {
        init(_config, state) {
            ...
            if (options.editor.docInfo.access_rights === "write") {
                ;
                this.spec.props.nodeViews["contributors_part"] = (node, view, getPos) => new ContributorsPartView(node, view, getPos);
            }
        ...
        apply(tr, prev, _oldState, state) {
            ...
            if (tr.docChanged) {
                contributorsPartPositions = contributorsPartPositions.map(range => ({
            ...
            if (tr.selectionSet) {
                ...
                const oldDropUpDeco = decos.find(undefined, undefined, spec => spec.id === "contributorDropUp");
                ...
                if (state.selection instanceof NodeSelection &&
                    ...
                    const dropUpDeco = Decoration.widget(state.selection.from, createDropUp(state.selection, options.editor.view), {
                        side: -1,
                        stopEvent: event => {
                            // Get drop-up element to check if it exists in DOM
                            const dropUpEl = document.querySelector(".drop-up-outer");
                            if (!dropUpEl) {
                            ...
                            if (event.type === "mousedown" ||
                                event.type === "click") {
                                return !!event.target.closest(".drop-up-outer");
                            }
                            ...
                            if (event.type === "keydown") {
                                ...
                                const isTargetInDropUp = event.target.closest(".drop-up-outer");
                                const dropUpHasFocus = dropUpEl.contains(document.activeElement);
                                ...
                                if (keysHandled.includes(event.key) &&
    ...
    props: {
        ...
        handleKeyDown(view, event) {
            const isContributorSelected = view.state.selection instanceof NodeSelection &&
                view.state.selection.node.type.name === "contributor";
            const dropUpEl = document.querySelector(".drop-up-options");
            const dropUpHasFocus = dropUpEl && dropUpEl.contains(document.activeElement);
            ...
            if (event.key === " " && isContributorSelected) {
                const idTypes = view.editor.mod
                    .documentTemplate?.currentTemplate?.id_types || [];
                const selection = view.state.selection;
                const dialog = new ContributorDialog(selection.$anchor.parent, view, selection.node.attrs, idTypes);
                dialog.init();
                return true;
            }
    ...
    appendTransaction: (trs, oldState, newState) => {
        ...
        const contributorsPartRange = pluginState.contributorsPartPositions.find(range => selectionPos > range.start && selectionPos < range.end);
        if (contributorsPartRange) {
            ...
            if (selectionPos + 1 === contributorsPartRange.end) {
                ...
                if (oldSelectionPos < selectionPos) {
                    const newSelection = nextSelection(newState, contributorsPartRange.end, 1);
                    if (!newSelection) {
            ...
            const selectedNodePos = oldSelectionPos < selectionPos
                ? selectionPos
                : selectionPos - 1;
            if (selectedNodePos === contributorsPartRange.start) {
                ...
                const newSelection = nextSelection(newState, selectedNodePos, -1);
                if (!newSelection) {
                    // Cannot find a location. Give up.
                    return;
                }
                ...
                return newState.tr.setSelection(NodeSelection.create(newState.doc, selectedNodePos));
            }
        }
        return null;
    }
