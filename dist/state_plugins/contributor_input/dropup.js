...
class ContributorDropUp {
    ...
    createDropUp() {
        ...
        const optionsHTML = this.menuOptions
            .map((option, index) => `<li class="drop-up-option ${option.className}"
                       data-index="${index}"
    ...
    openContributorDialog() {
        ...
        const idTypes = this.view.editor.mod.documentTemplate?.currentTemplate?.id_types ||
            [];
        const dialog = new ContributorDialog(this.parentNode, this.view, this.selection.node.attrs, idTypes);
        dialog.init();
    }
    /**
    ...
    closeDropUp() {
        ...
        const tr = this.view.state.tr.setSelection(Selection.near(this.view.state.doc.resolve(this.selection.from)));
