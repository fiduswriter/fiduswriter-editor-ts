...
export const footnoteMarkersPlugin = (options) => new Plugin({
    key,
    state: {
        init(_config, state) {
            const fnMarkers = [];
        ...
        apply(tr, _prev, oldState, state) {
            ...
            const remote = tr.getMeta("remote"), fromFootnote = tr.getMeta("fromFootnote"), ranges = getAddedRanges(tr), deletedFootnotesIndexes = [];
            fnMarkers = fnMarkers
                .map(marker => ({
            ...
            deletedFootnotesIndexes.forEach(index => options.editor.mod.footnotes.fnEditor.removeFootnote(index, footTr));
            ranges.forEach(range => {
                ...
                if (newFootnotes.length) {
                    ...
                    if (remote) {
                        newFootnotes = newFootnotes.filter(newMarker => !fnMarkers.find(oldMarker => oldMarker.from === newMarker.from));
                    }
                    else {
                        newFootnotes.forEach((footnote, index) => {
                            const fnContent = state.doc.nodeAt(footnote.from).attrs.footnote;
                            options.editor.mod.footnotes.fnEditor.renderFootnote(fnContent, offset + index, footTr);
                        });
                    }
                    fnMarkers = fnMarkers
                        .concat(newFootnotes)
...
//# sourceMappingURL=footnote_markers.js.map