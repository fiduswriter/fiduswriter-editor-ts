...
const MathDialogCtor = MathDialog;
/**
...
function inlineEditorPlugin(options) {
    ...
    return new Plugin({
        ...
        appendTransaction: (trs, oldState, newState) => {
            ...
            const tracked = (tr) => amendTransaction(tr, newState, editor).setMeta(key, {
                action: "deactivate"
            });
            ...
            if (oldPluginState?.active) {
                if (trs.some(tr => tr.selectionSet) &&
                    ...
                    if (latex.trim()) {
                        ...
                        if (oldPluginState.isEdit) {
                            const existingNode = newState.doc.nodeAt(oldPluginState.mathNodePos);
                            return tracked(newState.tr.replaceWith(oldPluginState.mathNodePos, oldPluginState.mathNodePos +
                                (existingNode?.nodeSize || 1), equationNode));
                        }
                        else {
                            return tracked(newState.tr.insert(oldPluginState.from, equationNode));
                        }
                    }
                    else {
                        return newState.tr.setMeta(key, { action: "deactivate" });
...
export const inlineMathPlugin = (options) => {
    ...
    const enabled = appConfig.config?.user?.preferences?.inline_math === true &&
