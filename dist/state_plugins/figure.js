...
export const figurePlugin = (options) => new Plugin({
    key,
    state: {
        init(_config, _state) {
            const self = this;
            if (options.editor.docInfo.access_rights === "write") {
                self.spec.props.nodeViews["figure"] = (node, view, getPos) => new FigureView(node, view, getPos, options);
            }
            self.spec.props.nodeViews["figure_caption"] = (node, view, getPos) => new FigureCaptionView(node, view, getPos, options);
            return {};
        },
        apply(_tr, prev) {
            return prev;
