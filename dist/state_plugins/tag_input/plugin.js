...
import { nextSelection, submitTag, tagInputReferences } from "@fiduswriter/document/state_plugins/tag_input";
...
export const tagInputPlugin = (options) => new Plugin({
    key,
    state: {
        init(_config, state) {
            const self = this;
            if (options.editor.docInfo.access_rights === "write") {
                self.spec.props.nodeViews["tags_part"] = (node, view, getPos) => new TagsPartView(node, view, getPos);
            }
    ...
    props: {
        ...
        handleDOMEvents: {
            mousedown(view, event) {
                ...
                const tagInputContainer = activeElement?.closest(".tag-input");
                ...
                const clickedOnTag = target.closest(".tag") ||
                ...
                if (tagInputContainer &&
                    ...
                    if (refs) {
                        ...
                        if (!clickedOnTag) {
                            ...
                            if (clickPos) {
                                const tr = view.state.tr.setSelection(TextSelection.create(view.state.doc, clickPos.pos));
                                view.dispatch(tr);
                                view.focus();
                            }
                        }
                            ...
                            if (clickPos) {
                                ...
                                try {
                                    const node = view.state.doc.nodeAt(clickPos.pos);
                                    if (node && node.type.name === "tag") {
                                        const tr = view.state.tr.setSelection(NodeSelection.create(view.state.doc, clickPos.pos));
                                        view.dispatch(tr);
                                    }
                                    else {
                                        // If not directly on a tag, try the position before
                                        const nodeBefore = view.state.doc.nodeAt(clickPos.pos - 1);
                                        if (nodeBefore &&
                                            nodeBefore.type.name === "tag") {
                                            const tr = view.state.tr.setSelection(NodeSelection.create(view.state.doc, clickPos.pos - 1));
                                            view.dispatch(tr);
                                        }
                                    }
                                }
    ...
    appendTransaction: (trs, oldState, newState) => {
        ...
        const tagsPartRange = pluginState.tagsPartPositions.find(range => selectionPos > range.start && selectionPos < range.end);
        if (tagsPartRange) {
            ...
            if (selectionPos + 1 === tagsPartRange.end) {
                ...
                if (oldSelectionPos < selectionPos) {
                    const newSelection = nextSelection(newState, tagsPartRange.end, 1);
                    if (!newSelection) {
