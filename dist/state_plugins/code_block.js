...
class CodeBlockView {
    ...
    constructor(node, view, getPos, options) {
        ...
        preElement.classList.forEach((className) => this.dom.classList.add(className));
        ...
        if (event.type === "mousedown") {
            ...
            if (composedPath.includes(this.menuButton)) {
                ...
                const contentMenu = new ContentMenu({
                    ...
                    menuPos: {
                        X: Number.parseInt(mouseEvent.pageX) + 20,
                        Y: Number.parseInt(mouseEvent.pageY) - 100
                    },
                    onClose: () => {
                        this.view.focus();
                    }
            ...
            else if (composedPath.includes(this.dom) &&
                !composedPath.find(el => el instanceof Element &&
                    el.matches &&
                    el.matches("code"))) {
                stopped = true;
                const tr = this.view.state.tr;
    ...
    update(node) {
        ...
        if (node.attrs.language !== oldNode.attrs.language) {
            const existingBadge = this.dom.querySelector(".code-block-language-badge");
            if (existingBadge) {
        ...
        if (node.attrs.category !== oldNode.attrs.category ||
...
export const codeBlockPlugin = (options) => new Plugin({
    key,
    state: {
        init(_config, _state) {
            const self = this;
            if (options.editor.docInfo.access_rights === "write") {
                self.spec.props.nodeViews["code_block"] = (node, view, getPos) => new CodeBlockView(node, view, getPos, options);
            }
            return {};
        },
        apply(_tr, prev) {
