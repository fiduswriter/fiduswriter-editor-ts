...
function commentContentTemplate({ serialized, isGlobal }) {
    if (isGlobal || serialized.text.length <= COMMENT_SHOW_MORE_THRESHOLD) {
        return `<p class="comment-p">${serialized.html}</p>`;
    }
    const truncatedText = `${escapeText(serialized.text.slice(0, COMMENT_TRUNCATE_LENGTH - 1))}…`;
...
/** A template for an answer to a comment */
const answerCommentTemplate = ({ answer, author, commentId, activeCommentAnswerId, active, user, isGlobal }) => {
...
const singleCommentTemplate = ({ comment, author, active, editComment }) => {
    ...
    return `<div class="comment-item">
        ...
        ? `<a type="button" class="comment-expand-compress show-more-less">${gettext("show more")}</a>`
        : ""}
                ${!active && (comment.answers?.length || 0) > 0
...
const firstCommentTemplate = ({ comment, author }) => `<div class="comment-item">
        <div class="comment-user">
            ${author ? avatarTemplate({ user: author }) : '<span class="fw-string-avatar"></span>'}
            <h5 class="comment-user-name">${escapeText(author?.name || comment.username)}</h5>
            <p class="comment-date">${localizeDate(comment.date)}</p>
...
const commentTemplate = ({ comment, view, active, editComment, activeCommentAnswerId, user, docInfo, filterOptions }) => {
    ...
    const author = comment.user === docInfo.owner?.id
        ? docInfo.owner
        : docInfo.owner?.contacts.find(contact => contact.id === comment.user && contact.type === "user"), assignedUser = comment.assignedUser
        ...
        : false, assignedUsername = assignedUser ? assignedUser.name : false;
    return `
...
${comment.comment.length === 0
        ...
        : singleCommentTemplate({ comment, author, active, editComment })}
    ${assignedUsername
        ? `<div class="assigned-user">${gettext("Assigned to")} <em>${escapeText(assignedUsername || "")}</em></div>`
        : ""}
    ${comment.answers
        ? comment.answers
            .map(answer => answerCommentTemplate({
            answer,
            author: answer.user === docInfo.owner?.id
                ? docInfo.owner
                : docInfo.owner?.contacts.find(contact => contact.id === answer.user &&
                    contact.type === "user"),
...
const trackTemplate = ({ type, data, node, active, docInfo, filterOptions }) => {
    ...
    const author = data.user === docInfo.owner?.id
        ? docInfo.owner
        : docInfo.owner?.contacts.find(contact => contact.id === data.user && contact.type === "user"), nodeActionType = `${type}_${node.type.name}`;
    return `
        <div class="margin-box track ${active ? "fw-active" : "inactive"}" data-type="${type}">
            <div class="track-${type}">
                <div class="comment-user">
                    ${author ? avatarTemplate({ user: author }) : '<span class="fw-string-avatar"></span>'}
                    <h5 class="comment-user-name">${escapeText(author?.name || data.username)}</h5>
                ...
                ${type === "format_change" ? formatChangeTemplate({ before: data.before, after: data.after || [] }) : type === "block_change" ? blockChangeTemplate({ before: data.before }, node) : ""}
            </div>
            ${docInfo.access_rights === "write"
            ...
            <div class="fw-marginbox-options fw-pulldown fw-right"><ul>
                <li>
                    ...
                    <div class="fw-pulldown marginbox-options-submenu">
                        ...
                        ${pastParticipants
            .map(user => `<li><span class="fw-pulldown-item margin-box-filter-comments-author${filterOptions.commentsAuthor === user.id ? " fw-selected" : ""}" data-id="${user.id}" title="${gettext("Show comments of ")} ${escapeText(user.name)}">
            ...
            .join("")}
                ...
                <li>
                    ...
                    <div class="fw-pulldown marginbox-options-submenu">
                        ...
                        ${pastParticipants
            .map(user => `<li><span class="fw-pulldown-item margin-box-filter-comments-assigned${filterOptions.assigned === user.id ? " fw-selected" : ""}" data-id="${user.id}" title="${gettext("Show comments of ")} ${escapeText(user.name)}">
                                    ${escapeText(user.name)}
                                </span></li>`)
            .join("")}
                        </ul>
            ...
            <div class="fw-marginbox-options fw-pulldown fw-right"><ul>
                <li>
                    ...
                    <div class="fw-pulldown marginbox-options-submenu">
                        ...
                        ${pastParticipants
            .map(user => `<li><span class="fw-pulldown-item margin-box-filter-track-author${filterOptions.trackAuthor === user.id ? " fw-selected" : ""}" data-id="${user.id}" title="${gettext("Show track changes of ")} ${escapeText(user.name)}">
                                    ${escapeText(user.name)}
                                </span></li>`)
            .join("")}
                        </ul>
...
export const globalCommentsTemplate = ({ globalComments, editComment, activeCommentAnswerId, user, docInfo, filterOptions }) => `<div id="global-comment-container"><div>${globalComments
    .map(mBox => commentTemplate({
    comment: mBox.data,
    view: mBox.view,
    active: mBox.active,
    activeCommentAnswerId,
...
export const marginBoxesTemplate = ({ marginBoxes, editComment, activeCommentAnswerId, user, docInfo, filterOptions }) => `<div id="margin-box-container"><div>${marginBoxes
    .map(mBox => {
    let returnValue = "";
    switch (mBox.type) {
        case "comment":
    ...
    .join("")}</div></div>`;
function getAssignees(docInfo) {
    return docInfo.owner ? [docInfo.owner, ...docInfo.owner.contacts] : [];
}
            ...
            : ""}
                <li>
                    ...
                    <div class="fw-pulldown marginbox-options-submenu">
                        ...
                        ${getAssignees(docInfo)
            .filter(contact => contact.type !== "userinvite")
            .map(contact => `<li><span class="fw-pulldown-item assign-comment" data-id="${comment.id}" data-user="${contact.id}" data-username="${escapeText(contact.name || "")}" title="${gettext("Assign comment to")} ${escapeText(contact.name || "")}">${escapeText(contact.name || "")}</span></li>`)
            .join("")}
                        </ul>
                    </div>
                </li>
