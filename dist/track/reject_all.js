...
export const rejectAll = (view, from = 0, to = false) => {
    ...
    view.state.doc.nodesBetween(from, to, (node, pos) => {
        ...
        if (node.attrs.track?.find(track => track.type === "insertion")) {
            ...
            const delStep = new ReplaceStep(map.map(Math.max(pos, from)), map.map(Math.min(pos + node.nodeSize, to)), Slice.empty);
            tr.step(delStep);
            ...
            tr.removeMark(map.map(Math.max(pos, from)), map.map(Math.min(pos + node.nodeSize, to)), view.state.schema.marks.deletion);
        }
        const formatChangeMark = node.marks.find(mark => mark.type.name === "format_change");
        if (node.isInline && !deletedNode && formatChangeMark) {
            ;
            formatChangeMark.attrs.before.forEach(oldMark => tr.step(new AddMarkStep(map.map(Math.max(pos, from)), map.map(Math.min(pos + node.nodeSize, to)), view.state.schema.marks[oldMark].create())));
            formatChangeMark.attrs.after.forEach(newMark => {
                const mark = node.marks.find(mark => mark.type.name === newMark);
                if (mark) {
                    tr.step(new RemoveMarkStep(map.map(Math.max(pos, from)), map.map(Math.min(pos + node.nodeSize, to)), mark));
                }
            });
            tr.step(new RemoveMarkStep(map.map(Math.max(pos, from)), map.map(Math.min(pos + node.nodeSize, to)), formatChangeMark));
        }
        if (!node.isInline && !deletedNode && node.attrs.track) {
            ...
            if (blockChangeTrack) {
                ...
                tr.setNodeMarkup(map.map(pos), view.state.schema.nodes[blockChangeTrack.before.type], Object.assign({}, node.attrs, blockChangeTrack.before.attrs, {
                    track
                }), node.marks);
            }
        }
...
//# sourceMappingURL=reject_all.js.map