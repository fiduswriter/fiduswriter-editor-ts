    ...
    tr.doc.nodesBetween(from, to, (node, pos) => {
        if (pos < from && node.type.name === "table_cell") {
            ...
            node.marks.find(mark => mark.type.name === "insertion" &&
                ...
                !mark.attrs.approved)) {
            const removeStep = new ReplaceStep(deletionMap.map(Math.max(from, pos)), deletionMap.map(Math.min(to, pos + node.nodeSize)), Slice.empty);
        ...
        else if (!node.attrs.track?.find(trackAttr => trackAttr.type === "deletion") &&
            !["bullet_list", "ordered_list"].includes(node.type.name)) {
            const insertionTrack = node.attrs.track?.find(trackAttr => trackAttr.type === "insertion" &&
            ...
            if (insertionTrack) {
                ...
                if (node.isTextblock && to < pos + node.nodeSize) {
                    ...
                    if (selectionBefore instanceof TextSelection) {
                        removeStep = new ReplaceStep(deletionMap.map(selectionBefore.$anchor.pos), deletionMap.map(to), Slice.empty);
                    }
                }
                else {
                    removeStep = new ReplaceStep(deletionMap.map(Math.max(from, pos)), deletionMap.map(Math.min(to, pos + node.nodeSize)), Slice.empty);
                }
...
function markWrapping(tr, pos, oldNode, newNode, user, date1) {
    const oldTrack = oldNode.attrs.track;
    if (!oldTrack || !newNode) {
        return;
    }
    let track = oldTrack.slice();
    let blockTrack = track.find(track => track.type === "block_change");
    if (blockTrack) {
        track = track.filter(track => track !== blockTrack);
        const before = blockTrack.before;
        if (before.type !== newNode.type.name ||
            before.attrs.level !== newNode.attrs.level) {
            blockTrack = {
                type: "block_change",
                user: user.id,
                username: user.username,
        ...
        const before = {
            ...
            attrs: Object.assign({}, oldNode.attrs)
        };
        if (before.attrs.id) {
            delete before.attrs.id;
        }
        if (before.attrs.track) {
            delete before.attrs.track;
        }
        blockTrack = {
            type: "block_change",
            user: user.id,
...
export function amendTransaction(tr, state, editor) {
    ...
    if (!tr.steps.length ||
        ...
        return trackedTransaction(tr, state, editor.user, !editor.view.state.doc.attrs.tracked &&
            !["write-tracked", "review-tracked"].includes(editor.docInfo.access_rights), Date.now() - editor.clientTimeAdjustment);
    }
}
export function trackedTransaction(tr, state, user, approved, date) {
    const newTr = state.tr, map = new Mapping(), date10 = Math.floor(date / 600000) * 10, // 10 minute interval
    ...
    tr.steps.forEach((originalStep, originalStepIndex) => {
        const step = originalStep.map(map), doc = newTr.doc;
        if (!step) {
            return;
        }
        if (step instanceof ReplaceStep) {
            const newStep = approved
                ...
                : step.slice.size && !cellDeleteTr
                    ? new ReplaceStep(step.to, // We insert all the same steps, but with "from"/"to" both set to "to" in order not to delete content. Mapped as needed.
                    step.to, step.slice, step.structure)
                    : false;
            ...
            if (newStep) {
                ...
                if (!trTemp.maybeStep(newStep).failed) {
                    ...
                    markInsertion(trTemp, newStep.from, mappedNewStepTo, user, date1, date10, approved);
            ...
            if (!approved && step.from !== step.to) {
                map.appendMapping(markDeletion(newTr, step.from, step.to, user, date1, date10));
            }
        }
        else if (approved) {
            newTr.step(step);
            ...
            if (step.from === step.gapFrom && step.to === step.gapTo) {
                ...
                map.appendMap(invertStep.getMap());
                map.appendMapping(markDeletion(newTr, step.from, step.gapFrom, user, date1, date10));
            }
            else if (step.slice.size === 2 &&
                ...
                if (oldNode?.attrs.track) {
                    markWrapping(newTr, step.from, oldNode, step.slice.content.firstChild, user, date1);
                }
            }
            else {
                newTr.step(step);
                ...
                ranges.forEach(range => doc.nodesBetween(range.from, range.to, (_node, pos) => {
                    if (pos < range.from) {
                        return true;
                    }
            ...
            doc.nodesBetween(step.from, step.to, (node, pos) => {
                ...
                if (["em", "strong", "underline"].includes(step.mark.type.name) &&
                    ...
                    if (formatChangeMark) {
                        if (formatChangeMark.attrs.before.includes(step.mark.type.name)) {
                            before = formatChangeMark.attrs.before.filter(markName => markName !== step.mark.type.name);
                ...
                if (["em", "strong", "underline"].includes(step.mark.type.name) &&
                    ...
                    if (formatChangeMark) {
                        if (formatChangeMark.attrs.after.includes(step.mark.type.name)) {
                            after = formatChangeMark.attrs.after.filter(markName => markName !== step.mark.type.name);
                            before = formatChangeMark.attrs.before;
                        }
                        else {
                            after = formatChangeMark.attrs.after;
...
//# sourceMappingURL=amend_transaction.js.map