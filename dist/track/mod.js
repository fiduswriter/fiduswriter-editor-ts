...
import { deactivateAllSelectedChanges, setSelectedChanges } from "../state_plugins/track/index.js";
...
export class ModTrack {
        ...
        this.editor.dom.addEventListener("click", event => {
            ...
            switch (true) {
                case findTarget(event, ".track-accept", el): {
                    ...
                    accept(el.target.dataset.type, box.pos, box.view === "main"
                        ? this.editor.view
                    ...
                    if (newBox) {
                        ;
                        this.editor.mod.track.activateTrack(newBox.view, newBox.type, newBox.pos);
                    }
                    break;
                }
                case findTarget(event, ".track-reject", el): {
                    ...
                    const box = this.editor.mod.marginboxes.marginBoxes[boxNumber];
                    reject(el.target.dataset.type, box.pos, box.view === "main"
                        ? this.editor.view
                        : this.editor.mod.footnotes.fnEditor.view);
                    // Activate the next margin box with the same number
                    const newBox = this.editor.mod.marginboxes.marginBoxes[boxNumber];
    ...
    activateTrack(viewName, type, pos) {
        ...
        const view = viewName === "main"
            ? this.editor.view
            : this.editor.mod.footnotes.fnEditor.view;
        const otherView = viewName === "main"
            ? this.editor.mod.footnotes.fnEditor.view
            : this.editor.view;
