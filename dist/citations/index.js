...
function namePartToString(part) {
...
export class ModCitations {
    ...
    init() {
         ...
         */
        this.editor.dom.insertAdjacentHTML("beforeend", '<style type="text/css" id="footnote-numbering-override"></style>');
    ...
    layoutCitations() {
        ...
        if (emptyCitations.length) {
            const settings = this.editor.view.state.doc.attrs, bibliographyHeader = settings.bibliography_header[settings.language] ||
                BIBLIOGRAPHY_HEADERS[settings.language];
            this.citRenderer = new RenderCitations(document.getElementById("paper-editable"), settings.citationstyle, bibliographyHeader, this.editor.mod.db.bibDB, this.editor.app.csl, false, settings.language);
            this.citRenderer
                .init()
    ...
    recoverAfterCiteProcCrash() {
        console.warn("CitationProcessor crashed. Falling back to simplified citation rendering.");
        // Create simplified citation texts for all citations
        const citationNodes = document.querySelectorAll("#paper-editable span.citation");
        citationNodes.forEach(citation => {
            try {
                // Get citation data from dataset
                const referencesData = JSON.parse(citation.dataset.references || "[]");
        ...
        if (docBibliography) {
            try {
                ...
                const allCitations = Array.from(document.querySelectorAll("#paper-editable span.citation"));
                const allRefs = new Set();
                allCitations.forEach(citation => {
                    try {
                        const refs = JSON.parse(citation.dataset.references || "[]");
                ...
                Array.from(allRefs)
                    ...
                    const authors = item.fields.author?.length
                        ? item.fields.author
                            .map((author) => authorName(author) || "Unknown")
                            .join(", ")
                        : "Unknown Author";
                ...
                if (!styleEl) {
                    this.editor.dom.insertAdjacentHTML("beforeend", '<style type="text/css" class="doc-bibliography-style"></style>');
    ...
    bindBibliographyClicks() {
        document.querySelectorAll("div.csl-entry").forEach((el, index) => {
            el.addEventListener("click", () => {
                const eID = Number.parseInt((this.citRenderer.fm.bibliography[0]?.entry_ids[index][0] ||
    ...
    layoutCitationsTwo() {
        ...
        if (this.citationType === "note") {
            ...
            if (emptyCitations) {
                ...
                const citationNodes = document.querySelectorAll("#document-editable span.citation"), citationsHTML = citRenderer.fm.citationTexts
                    .slice(0, citationNodes.length)
                    .map((citText) => `<div class="footnote-citation">${citText}</div>`)
                    .join("");
                ...
                citationNodes.forEach(citationNode => (citationNode.innerHTML =
    ...
    footnoteNumberOverride() {
        ...
        let outputCSS = "";
        if (this.citationType === "note") {
            let editorFootnoteCounter = 1, citationFootnoteCounter = 1, footnoteCounter = 1;
