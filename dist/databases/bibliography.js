...
export class ModBibliographyDB {
    ...
    saveBibEntries(tmpDB, isNew) {
        ...
        Object.keys(tmpDB).forEach(bibKey => {
            const reference = tmpDB[bibKey], bibId = Number.parseInt(bibKey);
    ...
    addReference(reference, id) {
        while (!id || (this.db && this.db[String(id)])) {
            id = randomID();
        }
    ...
    updateLocalReference(id, reference) {
        ...
        const preExisting = this.db && this.db[String(id)] ? true : false;
        this.db[String(id)] = reference;
        if (preExisting) {
    ...
    findReference(ref) {
        ...
        return Number(Object.keys(this.db).find(id => {
            const bib = this.db[id];
            return (bib.bib_type === ref.bib_type &&
