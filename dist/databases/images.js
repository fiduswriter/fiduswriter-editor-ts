...
export class ModImageDB {
    ...
    saveImage(imageData) {
        ...
        return this.mod.editor.app.apiConnectors.document
            ...
            const dbEntry = {
                ...
                original_file_type: json.original_file_type ||
    ...
    deleteLocalImage(id) {
        ...
        if (Object.keys(appImageDB.db).includes(String(id))) {
            ...
            this.reUploadImage(id, this.db[id].image, this.db[id].title, this.db[id].copyright).then(() => delete this.db[id], () => {
                delete this.db[id];
    ...
    reUploadImage(id, imageUrl, title, copyright) {
        ...
        const newPromise = new Promise((resolve, reject) => {
            ...
            get(imageUrl)
                ...
                this.mod.editor.app.imageDB.saveImage(x).then((newId) => {
                    ...
                    this.mod.editor.view.state.doc.descendants((node, pos) => {
                        if (node.type.name === "image" &&
                            ...
                            const attrs = Object.assign({}, node.attrs);
                            attrs["image"] = newId;
                            const nodeType = this.mod.editor.currentView.state
                                .schema.nodes["image"];
    ...
    findImage(imageData) {
        const id = Object.keys(this.db).find(id => this.db[Number.parseInt(id)]
