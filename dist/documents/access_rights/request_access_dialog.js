...
export class RequestAccessDialog {
    ...
    documentApi;
    isTokenUser;
    constructor(documentId, currentRights = "", documentApi, isTokenUser = false) {
    ...
    open() {
        ...
        if (!rightsOptions.length) {
            addAlert("error", gettext("You already have the highest possible access level."));
            return;
        }
        const defaultRights = rightsOptions.find(right => right.value === "write")?.value ||
            rightsOptions[rightsOptions.length - 1].value;
        const bodyText = this.currentRights
            ? gettext("Select a higher access level to request from the document owner.")
            : gettext("You do not have access to this document. Select the access level you would like to request from the document owner.");
        const dialog = new Dialog({
            ...
            <table class="fw-dialog-table">
                <tbody>
                    <tr>
                        <th><label for="request-access-rights">${gettext("Access level")}</label></th>
                        <td class="entry-field">
                            <select id="request-access-rights" class="fw-button fw-light fw-large">
                                ${rightsOptions
                .map(right => `<option value="${right.value}"${right.value === defaultRights ? " selected" : ""}>${right.label}</option>`)
                .join("")}
                            </select>
            ...
            buttons: [
                {
                    ...
                    classes: "fw-dark",
                    click: () => {
                        const rights = dialog.dialogEl.querySelector("#request-access-rights").value;
                        this.documentApi
                            .requestAccess({
                            ...
                            const data = json;
                            if (data.success) {
                                addAlert("success", gettext("Your access request has been sent to the document owner."));
                                dialog.close();
                            }
                            else {
                            ...
                            .catch(() => {
                            addAlert("error", gettext("Could not send access request."));
                        });
                    }
                },
                { type: "cancel" }
...
//# sourceMappingURL=request_access_dialog.js.map