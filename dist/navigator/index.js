...
export class ModNavigator {
    ...
    bindEvents() {
            ...
            switch (true) {
                case findTarget(event, "#navigator-button", el):
                    if (this.navigatorEl?.classList.contains("opened")) {
                        ...
                        this.openNavigator();
                        const activeHeading = this.navigatorEl?.querySelector("#navigator-list .active-heading a");
                        if (activeHeading) {
                            ;
                            activeHeading.focus();
                        }
                        else {
                            const firstFocusable = this.navigatorEl?.querySelector("#navigator-list [href]");
                            if (firstFocusable) {
                                ;
        ...
        this.editor.dom.addEventListener("keydown", event => {
            ...
            if (event.key === "Tab") {
                const focusableElements = navigatorEl.querySelectorAll('button, [href], input, [tabindex="0"]');
            ...
            if (navigatorFilterEl &&
                !navigatorFilterEl.classList.contains("fw-hide") &&
                (event.key === "ArrowDown" || event.key === "ArrowUp")) {
                event.preventDefault();
                const checkboxes = navigatorFilterEl.querySelectorAll('input[type="checkbox"]');
    ...
    closeNavigator() {
        ...
        if (this.lastSelectedTarget) {
            const target = this.lastSelectedTarget == "title"
    ...
    populateNavigator() {
        ...
        const title = this.editor.dom.querySelector("div.doc-title")?.textContent ||
        ...
        this.editor.view.state.doc.descendants((node, pos) => {
            if (node.attrs?.hidden) {
                ...
                if (pos <= currentPos) {
                    ...
                    items[items.length - 1] = Object.assign({}, items[items.length - 1], {
