...
import { copyrightTemplate, licenseInputTemplate, licenseSelectTemplate } from "./templates.js";
...
export class CopyrightDialog {
    ...
    getCurrentValue() {
        this.copyright = {};
        const holder = this.dialog.dialogEl.querySelector(".holder");
        this.copyright.holder = holder.value.length ? holder.value : false;
        const year = this.dialog.dialogEl.querySelector(".year");
        this.copyright.year = year.value.length
            ? Math.max(0, Math.min(Number.parseInt(year.value) || 0, 2100))
            : false;
        this.copyright.freeToRead = this.dialog.dialogEl.querySelector(".free-to-read:checked")
            ? true
            : false;
    ...
    bind() {
        this.licensesList = new InputList({
            ...
            renderItem: (license) => ({
                ...
                bind: el => {
                    ...
                    new TypeSwitch({
                        ...
                        render2: () => licenseInputTemplate({
                            url: license.url,
                            title: license.title
                        }),
                        onChange: () => {
                            // Restore focus to the license input after switching.
                            const focusable = el.querySelector(".fw-type-switch-input-inner input, .fw-type-switch-input-inner select");
                            if (focusable) {
                                ;
                                focusable.focus();
                            }
            ...
            getValue: (el) => {
                const licenseInput = el.querySelector(".fw-type-switch-input-inner");
                ...
                if (selectEl) {
                    ...
                    title = licenseInput.querySelector("input.license-title").value;
                }
                const startValue = el.closest("tr")?.querySelector(".license-start")?.value;
                const start = startValue || false;
