...
export class AddContactDialog {
    settings;
    contactsApi;
    constructor(settings, contactsApi) {
    ...
    init() {
        return new Promise(resolve => {
            const buttons = [
                {
                    ...
                    click: () => {
                        ...
                        userStrings
                            ...
                            .forEach(singleUserString => (chain = chain.then(responses => this.addContact(singleUserString).then(data => [...responses, data]))));
            ...
            const dialog = new Dialog({
                id: "add-new-contact",
                title: this.settings?.REGISTRATION_OPEN ||
    ...
    async addContact(userString) {
        ...
        if (status == 201) {
            ...
            document
                ...
                ?.insertAdjacentHTML("beforeend", `<div class="fw-warning" style="padding: 8px;">${escapeText(userString)}: ${responseHtml}</div>`);
            return cancelPromise();
        }
    }
}
//# sourceMappingURL=add_dialog.js.map